...
25. 找出字符串中第一个匹配项的下标（https://leetcode.cn/problems/find-the-index-of-the-first-occurrence-in-duplicate-string/）计算子字符串第一次出现的位置
...
class Solution:
    ...
    def cf_1326d2(ac=FastIO()):
        ...
        for _ in range(ac.read_int()):
            ...
            mid = s[i:j + 1]
            duplicate = KMP().find_longest_palindrome(s)
            s1 = mid[:duplicate]

            duplicate = KMP().find_longest_palindrome(s, "suffix")
            s2 = mid[-duplicate:]
            if len(s1) > len(s2):
                ac.st(s[:i] + s1 + s[j + 1:])
            else:
