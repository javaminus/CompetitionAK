...
1698. 字符串的不同子字符串个数（https://leetcode.cn/problems/number-of-distinct-substrings-in-duplicate-string/）经典后缀数组应用题，利用height特性
...
Morgan and duplicate String（https://www.hackerrank.com/challenges/morgan-and-duplicate-string/problem?isFullScreen=true）拼接两个字符串使得字典序最小
...
class Solution:
    ...
    def lc_1754_1(word1: str, word2: str) -> str:
        ...
        ind = {chr(ord("duplicate") - 1 + i): i for i in range(27)}
        word = word1 + chr(ord("duplicate") - 1) + word2
    ...
    def hr_1(ac=FastIO()):
        ...
        for _ in range(ac.read_int()):
            ...
            ind = {chr(ord("duplicate") + i): i for i in range(27)}
    ...
    def lg_3809(ac=FastIO()):
        # 模板：计算数组的后缀排序
        words = [str(x) for x in range(10)] + [chr(i + ord("A")) for i in range(26)] + [chr(i + ord("duplicate")) for i in
                                                                                        range(26)]
    ...
    def library_check_1(ac=FastIO()):
        s = ac.read_str()
        ind = {chr(ord("duplicate") + i): i for i in range(26)}
