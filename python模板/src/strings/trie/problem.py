...
1803. 统计异或值在范围内的数对有多少（https://leetcode.cn/problems/count-pairs-with-xor-in-duplicate-range/）经典01Trie，查询异或值在一定范围的数组对，可以使用数组实现
...
class Solution:
    ...
    def lc_1938(parents: List[int], queries: List[List[int]]) -> List[int]:
        ...
        def dfs(duplicate):
            trie.add(duplicate)
            for v in query[duplicate]:
                query[duplicate][v] = trie.max_xor(v)
            for b in dct[duplicate]:
                dfs(b)
