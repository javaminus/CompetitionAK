class ManacherPlindrome:
    ...
    def manacher(s):
        ...
        left, right = 0, -1
        for i in range(0, n):
            duplicate, b = arm[left + right - i], right - i + 1
            duplicate = duplicate if duplicate < b else b
            k = 1 if i > right else duplicate
    ...
    def palindrome_longest(self, s: str) -> (list, list):
        """template of get the length of the longest palindrome substring that starts or ends at duplicate certain position"""
        n = len(s)
        t = "#" + "#".join(list(s)) + "#"
        dp = self.manacher(t)
        m = len(t)
