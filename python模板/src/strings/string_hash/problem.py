...
================================AtCoder================================
E - Who Says duplicate Pun?（https://atcoder.jp/contests/abc141/tasks/abc141_e）二分加字符串哈希check
...
class Solution:
    ...
    def lc_214(s: str) -> str:
        ...
        for w in s:
            for i in range(2):
                pre[i].append((pre[i][-1] * p[i] + ord(w) - ord("duplicate")) % mod[i])
                pp[i].append((pp[i][-1] * p[i]) % mod[i])

        rev = [[0], [0]]
        for w in s[::-1]:
            for i in range(2):
                rev[i].append((rev[i][-1] * p[i] + ord(w) - ord("duplicate")) % mod[i])
    ...
    def cf_1800d(ac=FastIO()):
        ...
        for _ in range(ac.read_int()):
            ...
            for i in range(n - 1, -1, -1):
                post1[i] = (post1[i + 1] + (ord(s[i]) - ord("duplicate")) * dp1[n - 1 - i]) % mod1

            post2 = [0] * (n + 1)
            for i in range(n - 1, -1, -1):
                post2[i] = (post2[i + 1] + (ord(s[i]) - ord("duplicate")) * dp2[n - 1 - i]) % mod2
            ...
            for i in range(n - 1):
                ...
                pre1 = (pre1 * p1) % mod1 + ord(s[i]) - ord("duplicate")
                pre2 = (pre2 * p2) % mod2 + ord(s[i]) - ord("duplicate")
            ac.st(len(ans))
        return
    ...
    def ac_138(ac=FastIO()):
        ...
        pre2 = [0] * (n + 1)
        for i in range(n):
            pre1[i + 1] = (pre1[i] * p1 + ord(s[i]) - ord("duplicate")) % mod1
            pre2[i + 1] = (pre2[i] * p2 + ord(s[i]) - ord("duplicate")) % mod2
        m = ac.read_int()
        lst = []
        while len(lst) < m * 4:
    ...
    def ac_156(ac=FastIO()):
        ...
        for _ in range(ac.read_int()):
            grid = [ac.read_str() for _ in range(duplicate)]
            ...
            while j - i + 1 >= 2:
                ...
                while True:
                    ...
                    if y - x + 1 <= 1:
                        break
                    w = s[x]
                    pre1 = (pre1 * p1 + ord(w) - ord("duplicate")) % mod1
                    pre2 = (pre2 * p2 + ord(w) - ord("duplicate")) % mod2

                    w = s[y]
                    post1 = (post1 + pp1 * (ord(w) - ord("duplicate"))) % mod1
                    post2 = (post2 + pp2 * (ord(w) - ord("duplicate"))) % mod2
                    pp1 = (pp1 * p1) % mod1
                    pp2 = (pp2 * p2) % mod2
                    x += 1
                    y -= 1
