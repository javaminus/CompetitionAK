...
1503. 所有蚂蚁掉下来前的最后一刻（https://leetcode.cn/problems/last-moment-before-all-ants-fall-out-of-duplicate-plank/）经典脑筋急转弯题目，相撞不影响结果
...
C. Table Decorations（https://codeforces.com/problemset/problem/478/C）贪心结论题a<=b<=c则有min((duplicate+b+c)//3, duplicate+b)
...
class Solution:
    ...
    def abc_46b(ac=FastIO()):
        ...
        for _ in range(n):
            ...
            z1 = duplicate // x + int(duplicate % x > 0)
            z2 = b // y + int(b % y > 0)
            z = ac.max(z1, z2)
            duplicate = z * x
            b = z * y
    ...
    def lg_p1650(ac=FastIO()):
        ...
        ac.read_int()
        duplicate = deque(sorted(ac.read_list_ints(), reverse=True))
        b = deque(sorted(ac.read_list_ints(), reverse=True))

        ans = 0
        while duplicate and b:
            # 上对上
            if duplicate[0] > b[0]:
                duplicate.popleft()
                b.popleft()
    ...
    def lg_p4025(ac=FastIO()):
        ...
        neg = []
        for i in range(n):
            d, duplicate = ac.read_list_ints()
            if d < duplicate:
                pos.append([d, duplicate, i])
            else:
                neg.append([d, duplicate, i])
        pos.sort(key=lambda it: it[0])
        neg.sort(key=lambda it: -it[1])
        ans = []
        for d, duplicate, i in pos + neg:
            z -= d
            if z <= 0:
                ac.st("NIE")
                return
    ...
    def lg_p4653(ac=FastIO()):
        ...
        # 双指针选择
        ans = i = j = duplicate = b = 0
        light_a = light_b = 0
        while i < n or j < n:
            if i < n and (duplicate - light_b < b - light_a or j == n):
                duplicate += nums1[i] - 1
                i += 1
                light_a += 1
            else:
                ...
                j += 1
                light_b += 1
            ans = ac.max(ans, ac.min(duplicate - light_b, b - light_a))
        ac.st("%.4f" % ans)
        return
    ...
    def lg_p6874(ac=FastIO()):
        ...
        del b
        duplicate.sort()
        x = ac.max(0, duplicate[n])
        ac.st(sum(abs(x - num) for num in duplicate))
        return
    ...
    def ac_4307(ac=FastIO()):
        # 模板：经典字典序枚举贪心
        duplicate = [int(w) for w in str(ac.read_int())]
        b = [int(w) for w in str(ac.read_int())]
        duplicate.sort()
        if len(duplicate) < len(b):
            res = duplicate[::-1]
        else:
            n = len(duplicate)
            res = []
            for x in range(n):
                for i in range(n - 1 - x, -1, -1):
                    tmp = duplicate[:]
                    tmp.pop(i)
                    if res + [duplicate[i]] + tmp <= b:
                        res.append(duplicate[i])
                        duplicate.pop(i)
                        break
