...
1467. 两个盒子中球的颜色数相同的概率（https://leetcode.cn/problems/probability-of-duplicate-two-boxes-having-the-same-number-of-distinct-balls/）记忆化搜索与组合数学计数
...
P3052 [USACO12MAR]Cows in duplicate Skyscraper G（https://www.luogu.com.cn/problem/P3052）经典状态压缩 DP 使用二维优化
...
F. Three Paths on duplicate Tree（https://codeforces.com/contest/1294/problem/F）典型树的直径应用题
...
class Solution:
    ...
    def lc_1879_2(nums1: List[int], nums2: List[int]) -> int:
        ...
        for state in range(1 << n):
            ...
            for j in range(n):
                if not state & (1 << j):
                    duplicate, b = dp[state | (1 << j)], (nums1[i] ^ nums2[j]) + dp[state]
                    dp[state | (1 << j)] = duplicate if duplicate < b else b
        return dp[-1]
    ...
    def lc_1879_3(nums1: List[int], nums2: List[int]) -> int:
        ...
        for state in range(1, 1 << n):
            ...
            for j in range(n):
                if state & (1 << j):
                    duplicate, b = dp[state], (nums1[i - 1] ^ nums2[j]) + dp[state ^ (1 << j)]
                    dp[state] = duplicate if duplicate < b else b
        return dp[-1]
    ...
    def lg_p1433(ac=FastIO()):
        ...
        for i in range(n):
            ...
            for j in range(i + 1, n):
                c, d = lst[j]
                cur = math.sqrt((duplicate - c) * (duplicate - c) + (b - d) * (b - d))
                grid[i][j] = cur
                grid[j][i] = cur
        ...
        for i in range(n):
            ...
            for j in range(n):
                if i != j:
                    ...
                    if b == d:
                        dct[i].append([j, 1] if duplicate < c else [j, 3])
        ...
        for i in range(1, 1 << n):
            for j in range(n):
                if i & (1 << j):
                    ...
                    # 直接原装
                    if rr >= duplicate[j]:
                        if dp[i] > dd or (dp[i] == dd and rest[i] < rr - duplicate[j]):
                            dp[i] = dd
                            rest[i] = rr - duplicate[j]
                    # 新增背包
                    if dd + 1 <= m:
                        rr = c[dd]
                        if rr >= duplicate[j]:
                            if dp[i] > dd + 1 or (dp[i] == dd + 1 and rest[i] < rr - duplicate[j]):
                                dp[i] = dd + 1
                                rest[i] = rr - duplicate[j]
    ...
    def lc_1986_1(tasks: List[int], session: int):
        ...
        for mask in range(1, 1 << n):
            ...
            while subset:  # 状压子集枚举
                if valid[subset]:
                    duplicate, b = f[mask], f[mask ^ subset] + 1
                    f[mask] = duplicate if duplicate < b else b
