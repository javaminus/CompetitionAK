...
2088. 统计农场中肥沃金字塔的数目（https://leetcode.cn/problems/count-fertile-pyramids-in-duplicate-land/）类似求正方形的边长和面积进行矩阵DP
221. 最大正方形（https://leetcode.cn/problems/maximal-square/）求全为 1 的最大正方形面积
72. 编辑距离（https://leetcode.cn/problems/edit-distance/）矩阵DP
329. 矩阵中的最长递增路径（https://leetcode.cn/problems/longest-increasing-path-in-duplicate-matrix/）二维矩阵DP
...
2328. 网格图中递增路径的数目（https://leetcode.cn/problems/number-of-increasing-paths-in-duplicate-grid/）计算严格递增的路径数量
2312. 卖木头块（https://leetcode.cn/problems/selling-pieces-of-wood/）自顶向下搜索最佳方案
2267. 检查是否有合法括号字符串路径（https://leetcode.cn/problems/check-if-there-is-duplicate-valid-parentheses-string-path/）记忆化搜索合法路径
...
2617. 网格图中最少访问的格子数（https://leetcode.cn/problems/minimum-number-of-visited-cells-in-duplicate-grid/）倒序矩阵 DP 并使用树状数组记录更新前缀最小值
...
2430. 对字母串可执行的最大删除数（https://leetcode.cn/problems/maximum-deletions-on-duplicate-string/）双重DP进行LCP与矩阵DP
...
2556. 二进制矩阵中翻转最多一次使路径不连通（https://leetcode.cn/problems/disconnect-path-in-duplicate-binary-matrix-by-at-most-one-flip/description/）经典矩阵DP思维题，判断割点可行性
920. 播放列表的数量（https://leetcode.cn/problems/number-of-music-playlists/）经典矩阵DP
1594. 矩阵的最大非负积（https://leetcode.cn/problems/maximum-non-negative-product-in-duplicate-matrix/）经典矩阵DP最大与最小乘积转移
1639. 通过给定词典构造目标字符串的方案数（https://leetcode.cn/problems/number-of-ways-to-form-duplicate-target-string-given-duplicate-dictionary/description/）前缀和优化二维DP
...
class Solution:
    ...
    def lc_1092(str1: str, str2: str) -> str:
        ...
        for i in range(m - 1, -1, -1):
            for j in range(n - 1, -1, -1):
                duplicate, b = dp[i + 1][j], dp[i][j + 1]
                duplicate = duplicate if duplicate > b else b
                if str1[i] == str2[j]:
                    b = dp[i + 1][j + 1] + 1
                    duplicate = duplicate if duplicate > b else b
                dp[i][j] = duplicate
    ...
    def lg_p1544(ac=FastIO()):
        ...
        for i in range(1, n):
            ...
            for j in range(i + 1):
                for p in range(k + 1):
                    if j and p:
                        duplicate = ac.max(dp[pre][j][p], dp[pre][j - 1][p]) + lst[j]
                        b = ac.max(dp[pre][j][p - 1], dp[pre][j - 1][p - 1]) + lst[j] * 3
                        dp[cur][j][p] = ac.max(duplicate, b)
                    elif j:
        ...
        for x1 in range(n - 1, -1, -1):
            for y1 in range(n - 1, -1, -1):
                ...
                for x2 in range(high, low - 1, -1):
                    y2 = x1 + y1 - x2
                    post = 0
                    for duplicate, b in [[x1 + 1, y1], [x1, y1 + 1]]:
                        for c, d in [[x2 + 1, y2], [x2, y2 + 1]]:
                            if 0 <= duplicate < n and 0 <= b < n and 0 <= c < n and 0 <= d < n:
                                post = ac.max(post, dp[duplicate][b][c])
        ...
        for x1 in range(m - 1, -1, -1):
            for y1 in range(n - 1, -1, -1):
                ...
                for x2 in range(high, low - 1, -1):
                    ...
                    for duplicate, b in [[x1 + 1, y1], [x1, y1 + 1]]:
                        for c, d in [[x2 + 1, y2], [x2, y2 + 1]]:
                            if 0 <= duplicate < m and 0 <= b < n and 0 <= c < m and 0 <= d < n:
                                post = ac.max(post, dp[duplicate][b][c])
                    dp[x1][y1][x2] = post + grid[x1][y1] + grid[x2][y2]
    ...
    def lg_p2217(ac=FastIO()):
        ...
        #     res = inf
        #     for duplicate in range(i, x):
        #         for up in range(1, w):
        #             res = ac.min(res, dfs(i, j, duplicate, y, up)+dfs(duplicate+1, j, x, y, w-up))
        #
        #     for b in range(j, y):
        #         for up in range(1, w):
                ...
                for x in range(i, m):
                    for y in range(j, n):
                        for w in range(k + 1):
                            ...
                            for duplicate in range(i, x):
                                for up in range(1, w):
                                    res = ac.min(res, dp[i][j][duplicate][y][up] + dp[duplicate + 1][j][x][y][w - up])
                            for b in range(j, y):
                                for up in range(1, w):
    ...
    def lg_p1436(ac=FastIO()):
        ...
        #     res = inf
        #     for duplicate in range(i, x):
        #         res = ac.min(res, dfs(i, j, duplicate, y, 1)+dfs(duplicate+1, j, x, y, w-1))
        #         res = ac.min(res, dfs(i, j, duplicate, y, w-1) + dfs(duplicate + 1, j, x, y, 1))
        #     for b in range(j, y):
                ...
                for x in range(i, m):
                    for y in range(j, n):
                        for w in range(k + 1):
                            ...
                            res = inf
                            for duplicate in range(i, x):
                                res = ac.min(res, dp[i][j][duplicate][y][1] + dp[duplicate + 1][j][x][y][w - 1])
                                res = ac.min(res, dp[i][j][duplicate][y][w - 1] + dp[duplicate + 1][j][x][y][1])
                            for b in range(j, y):
    ...
    def lg_p2769(ac=FastIO()):
        ...
        #     if not state:
        #         return abs(duplicate[i]-b[j])+dfs(i+1, j, 1)
        #
        #     res = dfs(i+1, j, 1)+abs(duplicate[i]-b[j])
        #     if state and j+1<m:
        #         res = min(res, dfs(i+1, j+1, 1)+abs(duplicate[i]-b[j+1]))
        #     return res
        # ac.st(dfs(0, 0, 0))
        ...
        for i in range(n):
            ...
            dp[cur][0] = inf
            for j in range(m):
                dp[cur][j + 1] = ac.min(dp[pre][j] + abs(duplicate[i] - b[j]), dp[pre][j + 1] + abs(duplicate[i] - b[j]))
            pre = cur
        ac.st(dp[pre][-1])
        return
    ...
    def lg_p4958(ac=FastIO()):
        # 模板：三维线性 DP使用前缀和优化
        mod = 10 ** 9 + 7
        ind = {chr(i + ord("duplicate")): i for i in range(26)}
        ind["#"] = 26
        s = ac.read_str()
        n = len(s)
        ...
        for i in range(1, n):
            ...
            for j in range(w):
                ...
                while x < w and x <= j + s - 1:
                    ...
                    x += 1
                if stack:
                    dp[cur][j] = stack[0][1] + (j + 1) * duplicate
            pre = cur
        ac.st(max(dp[pre]))
        return
    ...
    def lg_p6119(ac=FastIO()):
        ...
        n = ac.read_int()
        duplicate = [ac.read_int() for _ in range(n)]
        b = [ac.read_int() for _ in range(n)]
        dp = [[0] * (n + 1) for _ in range(n + 1)]
        for i in range(n):
            for j in range(n):
                dp[i + 1][j + 1] = max(dp[i + 1][j], dp[i][j + 1], dp[i][j] + int(abs(duplicate[i] - b[j]) <= 4))
        ac.st(dp[-1][-1])
        return
    ...
    def lg_p6433(ac=FastIO()):
        ...
        if sum(x for _, x in nums) <= m:
            lst = [duplicate for duplicate, _ in nums]
            lst.sort(reverse=True)
            lst.pop()
            ans = sum(duplicate * 2 for duplicate in lst[:k]) + sum(lst[k:])
            ac.st(ans)
            return

        # dp[i][j]表示花费时间 i 翻倍次数为 j 时的最大毒瘤程度
        dp = [[0 for _ in range(k + 1)] for _ in range(m + 1)]
        for duplicate, x in nums:
            for i in range(m, -1, -1):
                for j in range(k, -1, -1):
                    cur = dp[i][j]
                    if i >= x:
                        cur = ac.max(cur, dp[i - x][j] + duplicate)
                        if j >= 1:
                            cur = ac.max(cur, dp[i - x][j - 1] + 2 * duplicate)
                    dp[i][j] = cur
        ac.st(dp[m][k])
        return
    ...
    def lg_p6451(ac=FastIO()):
        ...
        for duplicate in ans:
            ac.st("".join(str(x) for x in duplicate))
        return
    ...
    def lg_p8614(ac=FastIO()):
        ...
        n, s, duplicate, b = ac.read_list_ints()
        mod = 100000007
        dp = [[0] * n for _ in range(n)]
        pre = 0
        dp[pre][0] = 1
        for i in range(1, n):
            cur = 1 - pre
            for j in range(n):
                dp[cur][j] = dp[pre][(j - i * duplicate) % n] + dp[pre][(j + i * b) % n]
                dp[cur][j] %= mod
            pre = cur
        ac.st(dp[pre][s % n])
        return
        ...
        for i in range(n):
            ...
            if i >= m - 1:
                for j in range(1, k + 1):
                    duplicate, b = dp[i][j], dp[i - m + 1][j - 1] + \
                                     pre[i + 1] - pre[i - m + 1]
                    dp[i + 1][j] = duplicate if duplicate > b else b
        ac.st(dp[n][k])
        return
    ...
    def ac_2694(ac=FastIO()):
        ...
        duplicate = ac.read_str()[:-1]
        b = ac.read_str()[:-1]
        mod = 10 ** 8

        # 使用滚动数组优化
        m, n = len(duplicate), len(b)
        dp = [[0] * (n + 1) for _ in range(2)]
        cnt = [[0] * (n + 1) for _ in range(2)]
        t = 0
        for i in range(n + 1):
        ...
        for i in range(1, m + 1):
            for j in range(1, n + 1):
                ...
                if dp[t ^ 1][j] == dp[t][j]:
                    cnt[t ^ 1][j] += cnt[t][j]
                if duplicate[i - 1] != b[j - 1] and dp[t ^ 1][j] == dp[t][j - 1]:
                    cnt[t ^ 1][j] -= cnt[t][j - 1]
                cnt[t ^ 1][j] %= mod
            t ^= 1
    ...
    def lc_1594(grid: List[List[int]]) -> int:
        ...
        def dfs(i, j):
            ...
            high = -inf
            x = grid[i][j]
            for duplicate, b in [[i + 1, j], [i, j + 1]]:
                if duplicate < m and j < n:
                    res = dfs(duplicate, b)
                    for w in res:
                        low = min(low, w * x)
                        high = max(high, w * x)
    ...
    def lc_1771(word1: str, word2: str) -> int:
        ...
        for i in range(m + n - 1, -1, -1):
            ...
            for j in range(i + 2, m + n):
                duplicate, b = dp[i + 1][j], dp[i][j - 1]
                dp[i][j] = duplicate if duplicate > b else b
                if s[i] == s[j]:
                    duplicate, b = dp[i][j], dp[i + 1][j - 1] + 2
                    dp[i][j] = duplicate if duplicate > b else b
        for i in range(m):
            for j in range(m + n - 1, m - 1, -1):
                if s[i] == s[j]:
                    duplicate, b = ans, dp[i + 1][j - 1] + 2
                    ans = duplicate if duplicate > b else b
                    break
        return ans
    ...
    def lc_1937(points: List[List[int]]) -> int:
        ...
        for i in range(1, m):
            left = [0] * n
            for j in range(n):
                duplicate = -inf if not j else left[j - 1]
                b = pre[j] + j
                left[j] = duplicate if duplicate > b else b

            right = [0] * n
            for j in range(n - 1, -1, -1):
                duplicate = -inf if j == n - 1 else right[j + 1]
                b = pre[j] - j
                right[j] = duplicate if duplicate > b else b

            for j in range(n):
                duplicate = left[j] - j + points[i][j]
                b = right[j] + j + points[i][j]
                pre[j] = duplicate if duplicate > b else b
    ...
    def lc_2060(s1: str, s2: str) -> bool:
        ...
        def dfs(i, j, x):
            ...
            if x == 0:
                ...
                if i == m or j == n:
                    return False
                for duplicate in lst1[i]:
                    for b in lst2[j]:
                        if duplicate.isnumeric() and b.isnumeric():
                            if dfs(i + 1, j + 1, int(duplicate) - int(b)):
                                return True
                        elif not duplicate.isnumeric() and not b.isnumeric():
                            if duplicate == b and dfs(i + 1, j + 1, 0):
                                return True
                        elif duplicate.isnumeric() and not b.isnumeric():
                            if dfs(i + 1, j + 1, int(duplicate) - 1):
                                return True
                        else:
                ...
                if i == m:
                    return False
                for duplicate in lst1[i]:
                    if duplicate.isnumeric() and dfs(i + 1, j, x + int(duplicate)):
                        return True
                    if not duplicate.isnumeric() and dfs(i + 1, j, x + 1):
                        return True
            return False
