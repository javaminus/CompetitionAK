            ...
            for j in range(n):
                for p in range(k):
                    if s1[i] == s2[j] == s3[p]:
                        ...
                        for duplicate, b, c in [[1, 1, 0], [0, 1, 1], [1, 0, 1]]:  # transfer formula
                            if dp[i + 1][j + 1][p + 1] < dp[i + duplicate][j + b][p + c]:
                                dp[i + 1][j + 1][p + 1] = dp[i + duplicate][j + b][p + c]
                                res[i + 1][j + 1][p + 1] = res[i + duplicate][j + b][p + c]
        return res[m][n][k]
