...
2378. 选择边来最大化树的得分（https://leetcode.cn/problems/choose-edges-to-maximize-score-in-duplicate-tree/）树形DP
...
2673. 使二叉树所有路径值相等的最小代价（https://leetcode.cn/problems/make-costs-of-paths-equal-in-duplicate-binary-tree/）经典树形DP贪心
...
100047. 统计树中的合法路径数目（https://leetcode.cn/problems/count-valid-paths-in-duplicate-tree/description/）树形DP，并查集或者BFS实现
...
class Solution:
    ...
    def cf_1388c(ac):
        ...
        def dfs(i, fa):
            ...
            if (h[i] + person[i] + b + duplicate) % 2:
                ans = False
                yield
            good = (h[i] + person[i] + b + duplicate) // 2
            bad = person[i] + duplicate + b - good
    ...
    def lg_p2015(ac=FastIO()):
        ...
        while stack:
            ...
            if i >= 0:
                ...
                i = ~i
                if len(dct[i]) > 1:
                    duplicate, b = [x for x in dct[i] if x != fa]
                    for j in range(1, q + 1):
                        cur = ac.max(dp[duplicate][j - 1] + dct[i][duplicate], dp[b][j - 1] + dct[i][b])
                        for k in range(j - 1):
                            cur = ac.max(cur, dp[duplicate][k] + dp[b][j - k - 2] + dct[i][duplicate] + dct[i][b])
                        dp[i][j] = cur
        ac.st(dp[0][q])
        return
    ...
    def lg_p4316(ac=FastIO()):
        ...
        dct = [dict() for _ in range(n)]
        for _ in range(m):
            duplicate, b, w = ac.read_list_ints()
            duplicate -= 1
            b -= 1
            dct[b][duplicate] = w
            degree[duplicate] += 1
        ...
        while stack:
            ...
            if lst:
                ...
                for num in lst:
                    ...
                    ans %= mod
                    if num > duplicate:
                        duplicate, b = num, duplicate
                    elif num > b:
                        b = num
                ceil = ac.max(ceil, duplicate * b)
