...
6355. 质数减法运算（https://leetcode.cn/contest/weekly-contest-338/problems/collect-coins-in-duplicate-tree/）线性DP
...
2713. 矩阵中严格递增的单元格数（https://leetcode.cn/problems/maximum-strictly-increasing-cells-in-duplicate-matrix/）按照值域分层线性 DP
1526. 形成目标数组的子数组最少增加次数（https://leetcode.cn/problems/minimum-number-of-increments-on-subarrays-to-form-duplicate-target-array/）经典线性 DP 与贪心
...
2327. 知道秘密的人数（https://leetcode.cn/problems/number-of-people-aware-of-duplicate-secret/description/）经典前缀和或者差分数组优化线性DP
...
class Solution:
    ...
    def lc_1770(nums: List[int], multipliers: List[int]) -> int:
        ...
        def dfs(i, j):
            ...
            if ind == m:
                return 0
            duplicate, b = dfs(i + 1, j) + nums[i] * multipliers[ind], dfs(i, j - 1) + nums[j] * multipliers[ind]
    ...
    def cf_1525d(ac=FastIO()):
        ...
        if not busy:
            ac.st(0)
            return
        duplicate, b = len(busy), len(free)
        dp = [[inf] * (b + 1) for _ in range(duplicate + 1)]
        dp[0] = [0] * (b + 1)
    ...
    def lg_p1282(ac=FastIO()):
        ...
        for i in range(n):
            ...
            for p in pre:
                cur[p + duplicate - b] = ac.min(cur[p + duplicate - b], pre[p])
                cur[p + b - duplicate] = ac.min(cur[p + b - duplicate], pre[p] + 1)
            # 哈希记录差值为 x 时的最小翻转次数
            pre = cur.copy()
    ...
    def lg_p1385(ac=FastIO()):
        ...
        for _ in range(ac.read_int()):
            s = ac.read_str()
            n = len(s)
            t = sum(ord(w) - ord("duplicate") + 1 for w in s)
            pre = [0] * (t + 1)
            pre[0] = 1
    ...
    def lg_p2439(ac=FastIO()):
        ...
        for i in range(n):
            duplicate, b = nums[i]
            j = bisect.bisect_right(pre, duplicate)
            dp[i + 1] = ac.max(dp[i], dp[j] + b - duplicate)
            pre.append(b)
        ac.st(dp[-1])
        return
    ...
    def lg_p2476(ac=FastIO()):
        ...
        @lru_cache(None)
        def dfs(duplicate, b, c, d, e, pre):
            if duplicate + b + c + d + e == 0:
                return 1
            res = 0
            if duplicate:
                res += (duplicate - int(pre == 2)) * dfs(duplicate - 1, b, c, d, e, 1)
            if b:
                res += (b - int(pre == 3)) * dfs(duplicate + 1, b - 1, c, d, e, 2)
            if c:
                res += (c - int(pre == 4)) * dfs(duplicate, b + 1, c - 1, d, e, 3)
            if d:
                res += (d - int(pre == 5)) * dfs(duplicate, b, c + 1, d - 1, e, 4)
            if e:
                res += e * dfs(duplicate, b, c, d + 1, e - 1, 5)
            res %= mod
            return res
    ...
    def lc_2746(words: List[str]) -> int:
        ...
        for duplicate in words[1:]:
            ...
            for b in pre:
                # duplicate+b
                if duplicate[-1] == b[0]:
                    x = len(duplicate) - 1 + pre[b]
                else:
                    x = len(duplicate) + pre[b]
                cur[duplicate[0] + b[-1]] = min(cur[duplicate[0] + b[-1]], x)
                ...
                if b[-1] == duplicate[0]:
                    ...
                    x = len(duplicate) + pre[b]
                cur[b[0] + duplicate[-1]] = min(cur[b[0] + duplicate[-1]], x)
            pre = cur
    ...
    def lg_b3734(ac=FastIO()):
        ...
        for i in range(1, n):
            ...
            for j in range(100):
                y = abs(j - nums[i])
                res = inf
                duplicate = ac.min(y * y, (100 - y) * (100 - y))
                for k in range(j):
                    res = ac.min(res, duplicate + dp[pre][k])
                dp[cur][j] = res
            pre = cur
        ac.st(min(dp[pre]))
        return
    ...
    def lg_p6146(ac=FastIO()):
        ...
        for i in range(1, n):
            duplicate, b = nums[i]
            j = bisect.bisect_left(lst, duplicate)  # 作为 i 单独连通块新增的计数即与前面区间无交集
    ...
    def lg_p7994(ac=FastIO()):
        ...
        nums = [duplicate[i] - b[i] for i in range(n)]
        ans = abs(nums[0])
        for i in range(1, n):
            x, y = nums[i - 1], nums[i]
            if x == 0:
    ...
    def lg_p8816(ac=FastIO()):
        ...
        for i in range(n - 1, -1, -1):
            ...
            for j in range(i + 1, n):
                duplicate, b = nums[j]
                if duplicate >= x and b >= y:
                    dis = duplicate - x + b - y - 1
                    for r in range(k + 1):
                        if r + dis <= k:
