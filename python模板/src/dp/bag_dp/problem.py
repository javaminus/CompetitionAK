...
class Solution:
    ...
    def lg_p6567(ac=FastIO()):
        ...
        dp = [0] * (ceil + 1)
        dp[0] = 1
        for k, duplicate in nums:
            for b in BagDP().bin_split_1(duplicate):
                x = b * k
    ...
    def lg_p1284(ac=FastIO()):
        ...
        def check():
            # 三角形面积计算公式
            ss = (duplicate + b + c) / 2
            return (ss * (ss - duplicate) * (ss - b) * (ss - c)) ** 0.5
            ...
            for i in range(s // 2, -1, -1):
                for j in range(s // 2, -1, -1):
                    ...
                    if i >= num and dp[i - num][j]:
                        dp[i][j] = 1
        ans = -1
        for duplicate in range(s // 2 + 1):
            for b in range(s // 2 + 1):
                if dp[duplicate][b]:
                    c = s - duplicate - b
                    # 三角形合法判断公式
                    if b + c > duplicate > 0 and duplicate + c > b > 0 and duplicate + b > c > 0:
                        cur = check()
                        ans = ac.max(ans, cur)
        if ans == -1:
            ac.st(ans)
    ...
    def lg_p1441(ac=FastIO()):
        ...
        ans = 0
        s = sum(duplicate)
        for item in combinations(duplicate, n - m):
            dp = [0] * (s + 1)
            dp[0] = 1
            for num in item:
    ...
    def lg_p1541(ac=FastIO()):
        ...
        duplicate, b, c, d = cnt[1], cnt[2], cnt[3], cnt[4]
        dp = [[[[0] * (d + 1) for _ in range(c + 1)] for _ in range(b + 1)] for _ in range(duplicate + 1)]
        dp[0][0][0][0] = nums[0]
        ans = 0
    ...
    def lg_p1759(ac=FastIO()):
        ...
        # 同时记录时间与字典序最小的方案
        for i in range(n):
            duplicate, b, c = nums[i]
            for j in range(m, duplicate - 1, -1):
                for k in range(v, b - 1, -1):
                    t, p = dp[j - duplicate][k - b]
    ...
    def lg_p2306(ac=FastIO()):
        ...
        for duplicate in cnt:
            for b in cnt[duplicate]:
                for x in BagDP().bin_split_1(cnt[duplicate][b]):
                    for i in range(m, x * duplicate - 1, -1):
                        dp[i] = ac.max(dp[i], dp[i - x * duplicate] + x * b)
        ans = max(dp)
        if ans >= k:
            ac.st("yes")
        else:
    ...
    def lg_p2760(ac=FastIO()):
        ...
        for duplicate in range(m):
            for b in range(n):
                if grid[duplicate][b]:
                    v, w, s = (duplicate + 1 + b + 1) * 2, grid[duplicate][b], mat[duplicate][b]
                    for r in range(v):
                        stack = deque()
    ...
    def lg_p2938(ac=FastIO()):
        ...
        for i in range(1, d):
            dp = [0] * (m + 1)
            for j in range(s):
                duplicate, b = nums[j][i - 1], nums[j][i]
                if b > duplicate:
                    for p in range(duplicate, m + 1):
                        dp[p] = ac.max(dp[p], dp[p - duplicate] + b)
    ...
    def lg_p3983(ac=FastIO()):
        ...
        m = 10
        duplicate = [0] + ac.read_list_ints()
        for i in range(1, m + 1):
            for j in range(i + 1):
                duplicate[i] = ac.max(duplicate[i], duplicate[j] + duplicate[i - j])
        # 第二个背包计算船运载的最大盈利
        cost = [0] + [1, 3, 5, 7, 9, 10, 11, 14, 15, 17]
        dp = [0] * (n + 1)
        for i in range(1, m + 1):
            for j in range(i, n + 1):
                dp[j] = ac.max(dp[j], dp[j - i] + duplicate[i] - cost[i])
        ac.st(dp[-1])
        return
    ...
    def lg_p5662(ac=FastIO()):
        ...
        for i in range(1, t):
            dp = [0] * (m + 1)
            for j in range(n):
                b, duplicate = grid[i][j], grid[i - 1][j]
                if b > duplicate:
                    for x in range(duplicate, m + 1):
                        dp[x] = ac.max(dp[x], dp[x - duplicate] + b)
            # 注意此时的 m 更新值
            m = max(m - i + dp[i] for i in range(m + 1))
        ac.st(m)
        return
