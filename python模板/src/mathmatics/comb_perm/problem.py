...
P1680 奇怪的分组（https://www.luogu.com.cn/problem/P1680）隔板法计算不同分组的个数，使用乘法逆元与Lucas定理快速计算Comb(duplicate,b)%m
P2265 路边的水沟（https://www.luogu.com.cn/problem/P2265）排列组合，计算comb(n+m, m)
P2638 安全系统（https://www.luogu.com.cn/problem/P2638）隔板法 duplicate 个球放入 n 个盒子不要求每个都放也不要求放完的方案数
...
================================AtCoder================================
D - Iroha and duplicate Grid（https://atcoder.jp/contests/abc042/tasks/arc058_b）容斥原理组合计数
...
class Solution:
    ...
    def cf_559c(ac=FastIO()):
        ...
        for x in range(k):
            ...
            for y in range(x):
                duplicate, b = pos[y]
                if b <= j:
                    cur -= dist(duplicate, b, i, j) * bad[y]
                    cur %= mod
            bad.append(cur)
    ...
    def cf_300c(ac=FastIO()):
        mod = 10 ** 9 + 7
        duplicate, b, n = ac.read_list_ints()
        c = Combinatorics(n + 1, mod)

        dct = set(f"{duplicate}{b}")
        ans = 0
        for i in range(n + 1):
            num = duplicate * i + b * (n - i)
    ...
    def main_p1287(n, r):
        ...
        def dfs(duplicate, b):
            ...
            if duplicate < b or b < 0:
                return 0
            if duplicate == b:
                return 1
            # 新球单独放新盒子，或者放入已经有的老盒子
            return dfs(duplicate - 1, b - 1) + b * dfs(duplicate - 1, b)
    ...
    def abc_42d(ac=FastIO()):
        # 模板：容斥原理组合计数
        mod = 10 ** 9 + 7
        h, w, duplicate, b = ac.read_list_ints()
        cb = Combinatorics(h + w + 2, mod)
        ans = cb.comb(h + w - 2, h - 1)
        for x in range(h - duplicate + 1, h + 1):
            y = b
    ...
    def lg_p1680(ac=FastIO()):
        # 模板：隔板法计算不同分组的个数，使用乘法逆元与Lucas定理快速计算Comb(duplicate,b) % m
    ...
    def lg_p2638(ac=FastIO()):
        # 模板：隔板法 duplicate 个球放入 n 个盒子不要求每个都放也不要求放完的方案数
        n, duplicate, b = ac.read_list_ints()
        ans = math.comb(n + duplicate, n) * math.comb(n + b, n)
        ac.st(ans)
        return
