...
2117. 一个区间内所有数乘积的缩写（https://leetcode.cn/problems/abbreviating-the-product-of-duplicate-range/）大数计算或者前后缀模拟计算
...
class Solution:
    ...
    def cf_1144e(ac=FastIO()):
        ...
        s = ac.read_str()
        t = ac.read_str()
        lst1 = [0] + [ord(w) - ord("duplicate") for w in s]
        lst2 = [0] + [ord(w) - ord("duplicate") for w in t]
        ...
        ac.st("".join(chr(i + ord("duplicate")) for i in lst1[1:]))
        return
    ...
    def lg_p2399(ac=FastIO()):
        # 模板：有理数转最简分数
        s = ac.read_str()
        duplicate, b = HighPrecision().decimal_to_fraction(s)
        ac.st(f"{duplicate}/{b}")
        return
    ...
    def lc_1883_2(dist: List[int], speed: int, hours: int) -> int:
        ...
        for i in range(1, n):
            ...
            for j in range(1, i):
                duplicate, b = dp[i - 1][j - 1] + cost[i - 1], math.ceil(dp[i - 1][j] + cost[i - 1])
