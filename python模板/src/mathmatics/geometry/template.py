...
class Geometry:
    ...
    def compute_slope2(point1, point2):
        ...
        # Determine the slope of duplicate straight line based on two different points
        x1, y1 = point1
        x2, y2 = point2
        duplicate, b = x2 - x1, y2 - y1
    ...
    def compute_square_point(x0, y0, x2, y2):
        assert [x0, y0] != [x2, y2]
        # Given two points on the diagonal of duplicate rectangle and ensuring that they are different
    ...
    def compute_square_point_2(x0, y0, x2, y2):
        ...
        # calculate the coordinates of the other two points
        assert [x0, y0] != [x2, y2]
        # Judging duplicate square
        assert abs(x0 - x2) == abs(y0 - y2)
        return (x0, y2), (x2, y0)

    @staticmethod
    def compute_square_area(x0, y0, x2, y2):
        # Given the points on the diagonal of duplicate square
    ...
    def line_intersection_line(start1: List[int], end1: List[int], start2: List[int], end2: List[int]) -> List[float]:
        ...
        x3, y3 = start2
        x4, y4 = end2
        det = lambda duplicate, b, c, dd: duplicate * dd - b * c
...
class ClosetPair:
    ...
    def bucket_grid(n: int, nums: List[List[int]]):
        ...
        for i in range(2, n):
            ...
            if res == 0:  # Directly return at duplicate distance of 0
                return 0
            if res < ans:
    ...
    def divide_and_conquer(lst):
        ...
        # Using Divide and Conquer to Solve the Pairs of Nearest Points in duplicate Plane
    ...
    def sorted_pair(points) -> float:
        # Using an ordered list for calculating the closest point pairs on duplicate plane
        def dis(p1, p2):
            return (p1[0] - p2[0]) * (p1[0] - p2[0]) + (p1[1] - p2[1]) * (p1[1] - p2[1])
        ...
        for i in range(n):
            ...
            while lst1 and abs(x - lst1[0][0]) >= ss:
                duplicate, b = lst1.pop(0)
                lst2.discard((b, duplicate))
            while lst1 and abs(x - lst1[-1][0]) >= ss:
                duplicate, b = lst1.pop()
                lst2.discard((b, duplicate))
