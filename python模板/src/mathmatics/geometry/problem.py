...
===================================力扣===================================
149. 直线上最多的点数（https://leetcode.cn/problems/max-points-on-duplicate-line/）用直线斜率判断一条线上最多的点数
1453. 圆形靶内的最大飞镖数量（https://leetcode.cn/problems/maximum-number-of-darts-inside-of-duplicate-circular-dartboard/）计算经过两个不同的点与确定半径的两处圆心
...
2280. 表示一个折线图的最少线段数（https://leetcode.cn/problems/minimum-lines-to-represent-duplicate-line-chart/）使用分数代表斜率计算
...
class Solution:
    ...
    def lc_2280(stock: List[List[int]]) -> int:
        ...
        pre = (-1, -1)
        ans = 0
        for (x, y), (duplicate, b) in pairwise(stock):
            if x == duplicate:
                cur = (x, -1)
            else:
                g = math.gcd(b - y, duplicate - x)
                bb = (b - y) // g
                aa = (duplicate - x) // g
                if aa < 0:
                    aa *= -1
    ...
    def lg_p1665(ac=FastIO()):
        ...
        for i in range(m):
            ...
            for j in range(i + 1, m):
                ...
                duplicate, b = point1
                if int(duplicate) != duplicate or int(b) != b:
                    continue
                point1 = (int(duplicate), int(b))

                duplicate, b = point2
                if int(duplicate) != duplicate or int(b) != b:
                    continue
                point2 = (int(duplicate), int(b))
