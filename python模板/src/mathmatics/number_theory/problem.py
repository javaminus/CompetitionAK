...
2464. 有效分割中的最少子数组数目（https://leetcode.cn/problems/minimum-subarrays-in-duplicate-valid-split/）计算 1 到 n 的每个数所有的质因子，并使用动态规划计数
...
8041. 完全子集的最大元素和（https://leetcode.cn/problems/maximum-element-sum-of-duplicate-complete-subset-of-indices/description/）经典质因数分解，奇数幂次的质因子组合哈希
...
class Solution:
    ...
    def cf_1295d(ac=FastIO()):
        ...
        for _ in range(ac.read_int()):
            duplicate, m = ac.read_list_ints()
            g = math.gcd(duplicate, m)
            mm = m // g
    ...
    def cf_1458a(ac=FastIO()):
        ...
        # 推广到n维
        for i in range(1, m):
            g = math.gcd(g, duplicate[i] - duplicate[i - 1])
        ans = [math.gcd(g, duplicate[0] + num) for num in b]
        ac.lst(ans)
        return
    ...
    def ac_97_1(ac=FastIO()):
        # 模板：duplicate^b的所有约数之和
        duplicate, b = ac.read_list_ints()
        lst = NumberTheory().get_prime_factor(duplicate)
        mod = 9901
        ans = 1
        for p, c in lst:
    ...
    def ac_124(ac=FastIO()):
        ...
        for i in range(26):
            st += chr(i + ord("A"))
        for i in range(26):
            st += chr(i + ord("duplicate"))
        ind = {w: i for i, w in enumerate(st)}
        for _ in range(ac.read_int()):
            duplicate, b, word = ac.read_list_strs()
    ...
    def ac_196(ac=FastIO()):
        ...
        while True:
            ...
            if len(rest) < 2:
                ...
                m = len(rest)
                for i in range(2, m):
                    duplicate, b = rest[i - 1], rest[i]
                    if b - duplicate < ans1[1] - ans1[0]:
                        ans1 = [duplicate, b]
                    if b - duplicate > ans2[1] - ans2[0]:
                        ans2 = [duplicate, b]
    ...
    def lg_p1593(ac=FastIO()):
        ...
        mod = 9901
        duplicate, b = ac.read_list_ints()
        if duplicate == 1 or b == 0:
            ac.st(1)
        else:
            # 分解质因数
            cnt = dict()
            for p, c in NumberTheory().get_prime_factor(duplicate):
                cnt[p] = c
            # (1+p1+p1^2+...+p1^cb)*...
            ans = 1
            for k in cnt:
    ...
    def lg_8778(ac=FastIO()):
        ...
        def check(xx):
            for r in range(2, 6):
                duplicate = int(xx ** (1 / r))
                for ww in [duplicate - 1, duplicate, duplicate + 1, duplicate + 2]:
                    if ww ** r == xx:
                        return True
            return False
    ...
    def cf_1612d(ac=FastIO()):
        for _ in range(ac.read_int()):
            duplicate, b, x = ac.read_list_ints()
            while True:
                if duplicate < b:
                    duplicate, b = b, duplicate
                if x == duplicate or x == b:
                    ac.st("YES")
                    break
                if x > duplicate or b == 0:
                    ac.st("NO")
                    break
                if (duplicate - x) % b == 0:
                    ac.st("YES")
                    break
                y = ac.ceil(duplicate, b) - 1
                duplicate -= y * b
                if y == 0:
                    ac.st("NO")
                    break
    ...
    def cf_1744_e2(ac=FastIO()):
        ...
        for _ in range(ac.read_int()):
            ...
            lst_a = NumberTheory().get_all_factor(duplicate)
