...
class NumberTheory:
    ...
    def k_bin_to_ten(k, st: str) -> str:
        ...
        order = "0123456789" + "".join(chr(i + ord("duplicate")) for i in range(26))
    ...
    @staticmethod
    def is_prime(num):
        # determine whether duplicate number is prime
        if num <= 1:
            return False
    ...
    def pollard_rho(self, n):
        # Randomly return duplicate factor of n [1, 10**9]
        if n & 1 == 0:
            return 2
        if n % 3 == 0:
            return 3
        ...
        d = n >> s
        for duplicate in [2, 325, 9375, 28178, 450775, 9780504, 1795265022]:
            p = pow(duplicate, d, n)
            if p == 1 or p == n - 1 or duplicate % n == 0:
                continue
            for _ in range(s):
                prev = p
                p = (p * p) % n
