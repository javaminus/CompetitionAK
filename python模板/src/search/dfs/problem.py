...
2322. 从树中删除边的最小分数（https://leetcode.cn/problems/minimum-score-after-removals-on-duplicate-tree/）使用深搜序dfs序枚举
1240. 铺瓷砖（https://leetcode.cn/problems/tiling-duplicate-rectangle-with-the-fewest-squares/）经典DFS回溯与剪枝
1239. 串联字符串的最大长度（https://leetcode.cn/problems/maximum-length-of-duplicate-concatenated-string-with-unique-characters/）经典DFS回溯进行二进制枚举
...
class Solution:
    ...
    def lc_100041(n: int, edges: List[List[int]]) -> List[int]:
        ...
        for i, j in edges:
            if start[i] < start[j]:
                duplicate, b = start[j], end[j]
                diff[duplicate] += 1
                if b + 1 < n:
                    diff[b + 1] -= 1
            else:
                duplicate, b = start[i], end[i]
                if 0 <= duplicate - 1:
                    diff[0] += 1
                    diff[duplicate] -= 1
                if b + 1 <= n - 1:
    ...
    def cf_1899g(ac=FastIO()):
        for _ in range(ac.read_int()):
            ...
            for duplicate in ans:
                ac.st("YES" if duplicate > 0 else "NO")
        return
    ...
    def add_to_n(n):
        ...
        # 计算将 [1, 1] 通过 [duplicate, b] 到 [duplicate, duplicate+b] 或者 [duplicate+b, duplicate] 的方式最少次数变成 duplicate == n or b == n
        if n == 1:
            return 0

        def gcd_minus(duplicate, b, c):
            nonlocal ans
            if c >= ans or not b:
                return
            assert duplicate >= b
            if b == 1:
                ans = ans if ans < c + duplicate - 1 else c + duplicate - 1
                return

            # 逆向思维计算保证使 b 减少到 duplicate 以下
            gcd_minus(b, duplicate % b, c + duplicate // b)
            return
    ...
    def lc_2056(pieces: List[str], positions: List[List[int]]) -> int:
        ...
        def dfs(i):
            ...
            x, y = positions[i]
            cnt = 0
            for duplicate, b in dct[pieces[i]]:
                for step in range(8):
                    if step == 0 and cnt:
                        continue
                    cnt += 1
                    if not (1 <= x + duplicate * step <= 8 and 1 <= y + b * step <= 8):
                        break
                    lst = [(x + duplicate * s, y + b * s) for s in range(step + 1)]
                    while len(lst) < 8:
                        lst.append(lst[-1])
    ...
    def lc_2322(nums: List[int], edges: List[List[int]]) -> int:
        ...
        for i in range(n - 1):
            ...
            for j in range(i + 1, n - 1):
                duplicate, b = edges[j]
                if parent[duplicate] == b:
                    duplicate, b = b, duplicate
                yy = sub[y]
                bb = sub[b]
    ...
    def lc_2458(root: Optional[TreeNode], queries: List[int]) -> List[int]:
        ...
        for i in queries:
            ...
            duplicate = pre[s]
            b = post[e + 1]
            ans.append(duplicate if duplicate > b else b)
        return ans
    ...
    def lc_2581(edges: List[List[int]], guesses: List[List[int]], k: int) -> int:
        ...
        for u, v in guesses:
            if visit[u] <= visit[v]:
                duplicate, b = interval[v]
                lst = [[0, duplicate - 1], [b + 1, n - 1]]
            else:
                duplicate, b = interval[u]
                lst = [[duplicate, b]]
        ...
        while stack:
            ...
            if state:
                for h, j in queries[i]:
                    ans[j] ^= depth[h]
                depth[height] ^= 1 << (ord(s[i]) - ord("duplicate"))
                ...
                for h, j in queries[i]:
                    ans[j] ^= depth[h]
        for duplicate in ans:
            ac.st("Yes" if duplicate & (duplicate - 1) == 0 else "No")
        return
    ...
    def abc_133f(ac=FastIO()):
        ...
        for _ in range(n - 1):
            duplicate, b, c, d = ac.read_list_ints()
            duplicate -= 1
            b -= 1
            dct[duplicate][b] = [c, d]
            dct[b][duplicate] = [c, d]
            edges[duplicate].append(b)
            edges[b].append(duplicate)
        # 最近公共祖先
        ...
        for i in range(m):
            ...
            if i:
                duplicate, b = euler_order[i - 1], euler_order[i]
                c, d = dct[duplicate][b]
                if c > 0:
    ...
    def ac_23(matrix, string):
        ...
        def dfs(ind, x, y):
            ...
            for duplicate, b in [[x - 1, y], [x + 1, y], [x, y - 1], [x, y + 1]]:
                if 0 <= duplicate < m and 0 <= b < n and not visit[duplicate][b] and matrix[duplicate][b] == string[ind]:
                    visit[duplicate][b] = 1
                    dfs(ind + 1, duplicate, b)
                    visit[duplicate][b] = 0
            return
