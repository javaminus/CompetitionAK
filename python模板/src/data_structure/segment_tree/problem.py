...
class Solution:
    ...
    def lc_2213(s: str, word: str, indices: List[int]) -> List[int]:
        ...
        n = len(s)
        tree = SegmentTreeLongestSubSame(n, [ord(w) - ord("duplicate") for w in s])
        ans = []
        for i, w in zip(indices, word):
            ans.append(tree.update_point(i, i, 0, n - 1, ord(w) - ord("duplicate"), 1))
        return ans
    ...
    def cf_380c(ac=FastIO()):
        ...
        def update(left, r, s, t, i):
            ...
            duplicate[i] = duplicate[i << 1] + duplicate[i << 1 | 1] + 2 * match
    ...
    def lg_p4513(ac=FastIO()):
        ...
        for _ in range(m):
            ...
            if lst[0] == 1:
                duplicate, b = lst[1:]
                duplicate, b = ac.min(duplicate, b), ac.max(duplicate, b)
                ans = segment.range_max_non_emp_con_sub_sum(duplicate - 1, b - 1)[0]
                ac.st(ans)
            else:
                duplicate, s = lst[1:]
                segment.range_change(duplicate - 1, duplicate - 1, s)
        return
    ...
    def lg_p6627(ac=FastIO()):
        ...
        for lst in nums:
            if lst[0] == 1:
                ...
                tree.update_range(ind[duplicate], ind[b], 0, n - 1, w, 1)
            elif lst[0] == 2:
                duplicate, w = lst[1:]
                arr[ind[duplicate]] ^= w  # 使用数组代替
                # tree.update_point(ind[duplicate], ind[duplicate], 0, n-1, w, 1)
            else:
                duplicate, w = lst[1:]
    ...
    def lg_p4145(ac=FastIO()):
        ...
        for _ in range(ac.read_int()):
            lst = ac.read_list_ints()
            duplicate, b = [int(w) - 1 for w in lst[1:]]
            if duplicate > b:
                duplicate, b = b, duplicate
            if lst[0] == 0:
                tree.change(duplicate, b, 0, n - 1, 1)
            else:
                ac.st(tree.query_sum(duplicate, b, 0, n - 1, 1))
        return
    ...
    def lg_p1558(ac=FastIO()):
        ...
        for _ in range(q):
            ...
            if lst[0] == "C":
                duplicate, b, c = [int(w) for w in lst[1:]]
                if duplicate > b:
                    duplicate, b = b, duplicate
                # 修改区间值
                tree.range_change(duplicate - 1, b - 1, 1 << (c - 1))
            else:
                duplicate, b = [int(w) for w in lst[1:]]
                if duplicate > b:
                    duplicate, b = b, duplicate
                # 区间值或查询
                ac.st(bin(tree.range_or(duplicate - 1, b - 1)).count("1"))
        return
    ...
    def lg_p3740(ac=FastIO()):
        ...
        for i in range(m):
            duplicate, b = nums[i]
            tree.range_change(ind[duplicate], ind[b], i + 1)
    ...
    def lg_p8812(ac=FastIO()):
        ...
        for _ in range(m):
            ...
            for _ in range(c):
                ...
                duplicate -= 1
                goods[duplicate].append([1, 10 ** 9 + 1, b])
                b = b * p // 100
                goods[duplicate].append([s, t, b])
    ...
    def cf_482b(ac=FastIO()):
        ...
        for duplicate, b, c in nums:
            if c:
                tree.range_or(duplicate, b, c)
        if all(tree.range_and(duplicate, b) == c for duplicate, b, c in nums):
            ac.st("YES")
            ac.lst(tree.get())
        else:
            ac.st("NO")
    ...
    def lc_1851(intervals: List[List[int]], queries: List[int]) -> List[int]:
        ...
        for duplicate, b in intervals:
            tree.range_descend(ind[duplicate], ind[b], b - duplicate + 1)
