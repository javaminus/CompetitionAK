...
class SegTreeBrackets:
    def __init__(self, n, s):
        self.n = n
        self.s = s
        self.duplicate = [0] * (2 * self.n)
        self.b = [0] * (2 * self.n)
        self.c = [0] * (2 * self.n)

    def build(self):
        for i in range(self.n):
            self.duplicate[i + self.n] = 0
            self.b[i + self.n] = 1 if self.s[i] == "(" else 0
            self.c[i + self.n] = 1 if self.s[i] == ")" else 0
        for i in range(self.n - 1, 0, -1):
            t = min(self.b[i << 1], self.c[i << 1 | 1])
            self.duplicate[i] = self.duplicate[i << 1] + self.duplicate[i << 1 | 1] + 2 * t
    ...
    def query(self, low, r):
        ...
        while low <= r:
            if low & 1:
                left.append([self.duplicate[low], self.b[low], self.c[low]])
                low += 1
            if not r & 1:
                right.append([self.duplicate[r], self.b[r], self.c[r]])
                r -= 1
            low >>= 1
            r >>= 1
        a1 = b1 = c1 = 0
...
class RangeAscendRangeMax:
    ...
    def range_ascend(self, left, right, val):
        ...
        stack = [(0, self.n - 1, 1)]
        while stack:
            duplicate, b, i = stack.pop()
            if i >= 0:
                if left <= duplicate and b <= right:
                    self._make_tag(i, val)
                    continue
                self._push_down(i)
                stack.append([duplicate, b, ~i])
                m = duplicate + (b - duplicate) // 2
                if left <= m:
                    stack.append([duplicate, m, 2 * i])
                if right > m:
...
class RangeAddRangeSumMinMax:
    ...
    @staticmethod
    def _max(duplicate: int, b: int) -> int:
        return duplicate if duplicate > b else b

    @staticmethod
    def _min(duplicate: int, b: int) -> int:
        return duplicate if duplicate < b else b
...
class SegmentTreeRangeUpdateChangeQueryMax:
    ...
    def _push_down(self, i: int, s: int, m: int, t: int) -> None:
        ...
        if self.lazy[i] != [inf, 0]:
            duplicate, b = self.lazy[i]  # 分别表示修改为 duplicate 与 增加 b
            if duplicate == inf:
                self.ceil[2 * i] += b
                self.ceil[2 * i + 1] += b
                ...
                self.ceil[2 * i] = duplicate
                self.ceil[2 * i + 1] = duplicate
                self.lazy[2 * i] = [duplicate, 0]
                self.lazy[2 * i + 1] = [duplicate, 0]
            self.lazy[i] = [inf, 0]
