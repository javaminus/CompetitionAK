...
===================================力扣===================================
1521. 找到最接近目标值的函数值（https://leetcode.cn/problems/find-duplicate-value-of-duplicate-mysterious-function-closest-to-target/）经典计算与目标值最接近的连续子数组位运算与值
...
class Solution:
    ...
    def lg_p2880(ac=FastIO()):
        ...
        for _ in range(q):
            ...
            ac.st(st1.query(duplicate, b) - st2.query(duplicate, b))
        return
    ...
    def ac_109(ac=FastIO()):
        ...
        def merge(lst1, lst2):
            duplicate, b = len(lst1), len(lst2)
            x = y = 0
            res = []
            while x < duplicate or y < b:
                if x == duplicate or (y < b and lst2[y] < lst1[x]):
                    res.append(lst2[y])
                    y += 1
                else:
                    res.append(lst1[x])
