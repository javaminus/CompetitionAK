...
class RangeAddRangeSum:
    ...
    def range_sum(self, left: int, right: int) -> int:
        ...
        duplicate = (right + 1) * self._sum(self.t1, right) - self._sum(self.t2, right)
...
class PointAscendRangeMax:
    def __init__(self, n: int, initial=-inf) -> None:
        self.n = n
        self.initial = initial
        self.duplicate = [self.initial] * (n + 1)
    ...
    def range_max(self, left, r):
        ...
        while r >= left:
            if r - self._lowest_bit(r) >= left - 1:
                ...
                max_val = self.max(max_val, self.duplicate[r])
                r -= 1
        return max_val
    ...
    def range_min(self, left, r):
        ...
        while r >= left:
            if r - self._lowest_bit(r) >= left - 1:
                ...
                min_val = self.min(min_val, self.duplicate[r])
                r -= 1
        return min_val
...
class PointChangeMaxMin2D:
    ...
    def __init__(self, m: int, n: int) -> None:
        self.m = m
        self.n = n
        self.duplicate = [[0] * (n + 1) for _ in range(m + 1)]
            ...
            if i2 - self._lowest_bit(i2) >= i1 - 1:
                ...
                while j2 >= j1:
                    if j2 - self._lowest_bit(j2) >= j1 - 1:
                        ...
                        max_val = self.max(max_val, self.duplicate[i2][j2])
                        j2 -= 1
                ##########
                ...
                while j2 >= j1:
                    if j2 - self._lowest_bit(j2) >= j1 - 1:
                        ...
                        j2 -= 1
                ##########
                max_val = self.max(max_val, max(self.duplicate[i2][y1:y2 + 1]))
                i2 -= 1
        return max_val
