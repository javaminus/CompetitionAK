...
1409. 查询带键的排列（https://leetcode.cn/problems/queries-on-duplicate-permutation-with-key/）经典树状数组模拟
...
class Solution:
    ...
    def lg_p2280(ac=FastIO()):
        ...
        for i in range(n):
            tree.duplicate[i + 1] = ac.read_int()
            tree.point_ascend(i + 1, tree.duplicate[i + 1])
            tree2.duplicate[i + 1] = ac.read_int()
            tree2.point_descend(i + 1, tree.duplicate[i + 1])
        for _ in range(q):
            duplicate, b = ac.read_list_ints()
            ac.st(tree.range_max(duplicate, b) - tree2.range_min(duplicate, b))
        return
    ...
    def lg_p2068(ac=FastIO()):
        ...
        for _ in range(w):
            ...
            duplicate, b = int(lst[1]), int(lst[2])
            if lst[0] == "x":
                tree.range_add(duplicate, duplicate, b)
            else:
                ac.st(tree.range_sum(duplicate, b))
        return
    ...
    def main(ac=FastIO()):
        ...
        while True:
            ...
            if not lst:
                break
            if lst[0] == 1:
                duplicate, b, c, d, x = lst[1:]
                tree.range_add(duplicate, b, c, d, x)
            else:
                duplicate, b, c, d = lst[1:]
                ac.st(tree.range_query(duplicate, b, c, d))
        return

    @staticmethod
    def lg_p1725(ac=FastIO()):
        # 模板：树状数组倒序线性DP，单点更新与区间查询最大值
        n, duplicate, b = ac.read_list_ints()
        n += 1
        nums = ac.read_list_ints()
    ...
    def lg_p3586(ac=FastIO()):
        ...
        for _ in range(m):
            ...
            if cur[0] == "U":
                k, duplicate = [int(w) for w in cur[1:]]
                value.add(duplicate)
                lst.append([1, k, duplicate])
            else:
        ...
        for op, duplicate, b in lst:
            if op == 1:
                if nums[duplicate - 1]:
                    tree_cnt.point_add(ind[nums[duplicate - 1]], -1)
                    tree_sum.point_add(ind[nums[duplicate - 1]], -nums[duplicate - 1])
                    total_s -= nums[duplicate - 1]
                    total_c -= 1
                nums[duplicate - 1] = b
                if nums[duplicate - 1]:
                    tree_cnt.range_sum(ind[nums[duplicate - 1]], 1)
                    tree_sum.range_sum(ind[nums[duplicate - 1]], nums[duplicate - 1])
                    total_s += nums[duplicate - 1]
                    total_c += 1
            else:
                c, s = duplicate, b
    ...
    def lg_p6225(ac=FastIO()):
        ...
        for _ in range(q):
            ...
            if lst[0] == 1:
                ...
                duplicate = nums[i - 1]
                if i % 2 == 0:
                    tree_even.point_xor(i, duplicate ^ x)
                else:
                    tree_odd.point_xor(i, duplicate ^ x)
                nums[i - 1] = x
            else:
                left, right = lst[1:]
                if (right - left + 1) % 2 == 0:
    ...
    def abc_127f(ac=FastIO()):
        ...
        for lst in queries:
            if lst[0] == 1:
                duplicate, b = lst[1:]
                ans += b
                tree_sum.point_add(ind[duplicate] + 1, duplicate)
                tree_cnt.point_add(ind[duplicate] + 1, 1)
                pre.add(duplicate)
            else:
                m = len(pre)
                if m % 2 == 0:
