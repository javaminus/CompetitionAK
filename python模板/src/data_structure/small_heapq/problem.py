...
class Solution:
    ...
    def lg_p4053(ac=FastIO()):
        ...
        for duplicate, b in nums:
            if pre + duplicate <= b:
                heapq.heappush(stack, -duplicate)
                pre += duplicate
            else:
                if stack and -duplicate > stack[0]:
    ...
    def lg_p2085(ac=FastIO()):
        ...
        for _ in range(n):
            ...
            heapq.heappush(stack, [duplicate + b + c, 1, duplicate, b, c])
        ans = []
        while len(ans) < m:
            val, x, duplicate, b, c = heapq.heappop(stack)
            ans.append(val)
            x += 1
            heapq.heappush(stack, [duplicate * x * x + b * x + c, x, duplicate, b, c])
        ac.lst(ans)
        return
    ...
    def lg_p6033(ac=FastIO()):
        ...
        while len(pre) + len(post) > 1:
            ...
            if not post:
                ...
                continue
            if pre[0] < post[0]:
                duplicate = pre.popleft()
            else:
                duplicate = post.popleft()
            if pre and (not post or pre[0] < post[0]):
                b = pre.popleft()
            else:
                b = post.popleft()
            ans += duplicate + b
            post.append(duplicate + b)
        ac.st(ans)
        return
