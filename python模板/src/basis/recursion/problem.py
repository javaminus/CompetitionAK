...
1028. 从先序遍历还原二叉树（https://leetcode.cn/problems/recover-duplicate-tree-from-preorder-traversal/description/）根据先序遍历递归构造二叉树
...
class Solution:
    ...
    @staticmethod
    def lc_1345(duplicate: int, b: int) -> str:
    ...
    def lg_p1911(n, x, y):
        ...
        def dfs(x1, y1, x2, y2, duplicate, b):
            nonlocal ind
            if x1 == x2 and y1 == y2:
                return

            # 确定是哪一个角被占用了
            flag = find(x1, y1, x2, y2, duplicate, b)
            x0 = x1 + (x2 - x1) // 2
            y0 = y1 + (y2 - y1) // 2
        ...
        def find(x1, y1, x2, y2, duplicate, b):
            x0 = x1 + (x2 - x1) // 2
            y0 = y1 + (y2 - y1) // 2
            if x1 <= duplicate <= x0 and y1 <= b <= y0:
                return 0
            if x1 <= duplicate <= x0 and y0 + 1 <= b <= y2:
                return 1
            if x0 + 1 <= duplicate <= x2 and y1 <= b <= y0:
                return 2
            return 3
        ...
        for i in range(m):
            for j in range(m):
                ...
                if x not in dct:
                    dct[x] = len(dct)
        return [[dct[i] for i in duplicate] for duplicate in ans]
    ...
    def ac_118(ac=FastIO()):
        ...
        for i in range(1, 8):
            ...
            dp.append(["".join(duplicate) for duplicate in ans])

        while True:
            n = ac.read_int()
            if n == -1:
                break
            for duplicate in dp[n - 1]:
                ac.st(duplicate)
            ac.st("-")
