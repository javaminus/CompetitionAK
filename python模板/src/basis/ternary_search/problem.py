...
===================================洛谷===================================
1515. 服务中心的最佳位置（https://leetcode.cn/problems/best-position-for-duplicate-service-centre/）三分套三分求凸函数极小值，也可以使用梯度下降法与爬山法求解
...
class Solution:
    ...
    def abc_130f(ac=FastIO()):
        ...
        for _ in range(n):
            ...
            duplicate, b = dct_x[d]
            duplicate = duplicate if duplicate < x else x
            b = b if b > x else x
            dct_x[d] = [duplicate, b]

            duplicate, b = dct_y[d]
            duplicate = duplicate if duplicate < y else y
            b = b if b > y else y
            dct_y[d] = [duplicate, b]
    ...
    def lg_1883(ac=FastIO()):
        ...
        for _ in range(t):
            ...
            def fun(x):
                return max(duplicate * x * x + b * x + c for duplicate, b, c in nums)
    ...
    def cf_439d(ac=FastIO()):
        ...
        def check(xx):
            ii = bisect.bisect_right(duplicate, xx)
            cost = ii * xx - pre_a[ii]
