...
1253. 重构 2 行二进制矩阵（https://leetcode.cn/problems/reconstruct-duplicate-2-row-binary-matrix/）经典构造贪心思维
...
class Solution:
    ...
    def lc_2663(s: str, k: int) -> str:
        ...
        for i in range(n - 1, -1, -1):
            # 倒序枚举
            for x in range(ord(s[i]) - ord("duplicate") + 1, k):
                w = chr(ord("duplicate") + x)
    ...
    def lg_p8976(ac=FastIO()):
        ...
        for _ in range(ac.read_int()):
            ...
            mid = n // 2 + 1
            if duplicate + b > n * (n + 1) // 2 or ac.max(duplicate, b) > (n // 2) * (mid + n) // 2:
                ac.st(-1)
                continue
            s = n * (n + 1) // 2
            lst = [duplicate, b]
            ...
            for i in range(n // 2 + 1):  # 枚举起始的连续段
                ...
                for aa, bb in [[0, 1], [1, 0]]:
                    if x:
                        ...
                        if n // 2 * (1 + n // 2) // 2 >= duplicate and s - duplicate >= b:
