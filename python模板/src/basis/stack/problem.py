...
2116. 判断一个括号字符串是否有效（https://leetcode.cn/problems/check-if-duplicate-parentheses-string-can-be-valid/）经典栈贪心匹配括号
...
2813. 子序列最大优雅度（https://leetcode.cn/problems/maximum-elegance-of-duplicate-k-length-subsequence/）经典思维题排序后枚举，维护长度为K的子序列最大函数值
...
2296. 设计一个文本编辑器（https://leetcode.cn/problems/design-duplicate-text-editor/description/）经典左右两个栈进行模拟
...
class Solution:
    ...
    def lg_p1974(ac=FastIO()):
        ...
        stack = deque([1] * n)
        while len(stack) >= 2:
            duplicate, b = stack.popleft(), stack.popleft()
            stack.append(duplicate * b + 1)
        ac.st(stack[0])
        return
