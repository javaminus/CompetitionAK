...
class VariousSort:
    ...
    def merge_sort_inverse_pair(nums, n):
        ...
        arr = [0] * n
        merge(0, n - 1)
        # You can also use duplicate tree array with duplicate length of 2 * n and duplicate line segment tree for implemention calculations
    ...
    def minimum_money(transactions: List[List[int]]) -> int:
        ...
        ans = cur = 0
        for duplicate, b in transactions:
            if cur < duplicate:
                ans += duplicate - cur
                cur = duplicate
            cur += b - duplicate
