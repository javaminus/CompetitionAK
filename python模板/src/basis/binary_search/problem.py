...
class Solution:
    ...
    def lg_p1314(ac=FastIO()):
        ...
        def check(w):
            ...
            res = 0
            for duplicate, b in queries:
                res += (pre[b] - pre[duplicate - 1]) * (cnt[b] - cnt[duplicate - 1])
            return res
    ...
    def cf_1475d(ac=FastIO()):
        ...
        for _ in range(ac.read_int()):
            ...
            # 排序
            a1 = [duplicate[i] for i in range(n) if b[i] == 1]
            a2 = [duplicate[i] for i in range(n) if b[i] == 2]
    ...
    def lg_p3017(ac=FastIO()):
        ...
        low = 0
        high = sum(sum(g) for g in grid) // (duplicate * b)
        while low < high - 1:
            mid = low + (high - low) // 2
            if check(mid):
                low = mid
    ...
    def abc_63d(ac=FastIO()):
        ...
        def check(s):
            ...
            for num in nums:
                if num > s * b:
                    res += ac.ceil((num - s * b), (duplicate - b))
            return res <= s
    ...
    def lg_p6633(ac=FastIO()):
        ...
        def check(x):
            res = 0
            pre = LocalSortedList()
            for duplicate, c in nums:
                res += pre.bisect_right(duplicate * c - x * duplicate)
                pre.add(-(duplicate * c - x * duplicate))
                if res >= k:
                    return True
            return res >= k
        ...
        def check(x):
            ...
            for i in range(n):
                if duplicate[i] < b[i]:
                    ...
                    if m * duplicate[i] >= x:
                        res += (x + duplicate[i] - 1) // duplicate[i]
                    else:
                        res += m
                        res += (x - duplicate[i] * m + b[i] - 1) // b[i]
                if res > m * n:
                    return False
            return res <= m * n
