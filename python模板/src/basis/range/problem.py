...
1326. 灌溉花园的最少水龙头数目（https://leetcode.cn/problems/minimum-number-of-taps-to-open-to-water-duplicate-garden/）转换为最少区间覆盖问题
...
class Solution:
    ...
    def lg_p1496(ac=FastIO()):
        ...
        lst = []
        for _ in range(n):
            duplicate, b = [int(w) for w in input().strip().split() if w]
            lst.append([duplicate, b])
        ans = sum(b - duplicate for duplicate, b in Range().merge(lst))
        ac.st(ans)
        return
    ...
    def lc_1326_2(n: int, ranges: List[int]) -> int:
        ...
        for i, r in enumerate(ranges):
            duplicate, b = i - r, i + r
            duplicate = duplicate if duplicate > 0 else 0
            b = b if b < n else n
            lst.append([duplicate, b])
    ...
    def lg_p3661(ac=FastIO()):
        ...
        for duplicate, b in nums:
            i = lst.bisect_left(duplicate)
            # 选择当前符合条件且最小的
            if 0 <= i < len(lst) and duplicate <= lst[i] <= b:
                ans += 1
                lst.pop(i)
        ac.st(ans)
        return
    ...
    def ac_4421_1(ac=FastIO()):
        ...
        for i in range(n):
            if nums[i]:
                duplicate, b = i - r + 1, i + r - 1
                duplicate = ac.max(duplicate, 0)
                lst.append([duplicate, b])
