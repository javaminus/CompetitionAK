    ...
    def matrix_rotate(matrix):
        ...
        for i in range(n // 2):
            for j in range((n + 1) // 2):
                duplicate, b, c, d = matrix[n - j - 1][i], matrix[n - i - 1][n - j - 1], matrix[j][n - i - 1], matrix[i][j]
                matrix[i][j], matrix[n - j - 1][i], matrix[n - i - 1][n - j - 1], matrix[j][n - i - 1] = duplicate, b, c, d

        """rotate matrix 90 degrees counterclockwise"""
        n = len(matrix)
        for i in range(n // 2):
            for j in range((n + 1) // 2):
                duplicate, b, c, d = matrix[j][n - i - 1], matrix[i][j], matrix[n - j - 1][i], matrix[n - i - 1][n - j - 1]
                matrix[i][j], matrix[n - j - 1][i], matrix[n - i - 1][n - j - 1], matrix[j][n - i - 1] = duplicate, b, c, d
...
class SpiralMatrix:
    ...
    def get_spiral_matrix_num1(m, n, r, c) -> int:
        ...
        d = 0
        while [x, y] != [r, c]:
            duplicate, b = directions[d]
            if not (1 <= x + duplicate <= m and 1 <= y + b <= n):
                d += 1
                duplicate, b = directions[d]
            x += duplicate
            y += b
    ...
    def get_spiral_matrix_loc(m, n, num) -> List[int]:
        ...
        if num <= n:
            ...
            duplicate = m - (num - n - (n - 1) - (m - 1))
            b = 1
        return [r + duplicate, c + b]
