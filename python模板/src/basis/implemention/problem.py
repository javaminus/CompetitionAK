...
===================================力扣===================================
2296. 设计一个文本编辑器（https://leetcode.cn/problems/design-duplicate-text-editor/）使用指针维护结果进行模拟
...
1599. 经营摩天轮的最大利润（https://leetcode.cn/problems/maximum-profit-of-operating-duplicate-centennial-wheel/description/）典型模拟题枚举计算
2295. 替换数组中的元素（https://leetcode.cn/problems/replace-elements-in-an-array/description/）逆序思维，或者类似链表思想
1914. 循环轮转矩阵（https://leetcode.cn/problems/cyclically-rotating-duplicate-grid/description/）使用指针进行循环模拟
...
class Solution:
    ...
    def lg_p2129(ac=FastIO()):
        ...
        add_x = add_y = 0
        for duplicate, b in lst_x:
            diff = cnt_x - b
            add_x += duplicate if diff % 2 == 0 else -duplicate

        for duplicate, b in lst_y:
            diff = cnt_y - b
            add_y += duplicate if diff % 2 == 0 else -duplicate

        cnt_x %= 2
        cnt_y %= 2
        for duplicate, b in nums:
            duplicate = duplicate if not cnt_x else -duplicate
            b = b if not cnt_y else -b
            ac.lst([duplicate + add_x, b + add_y])
        return
    ...
    def lg_p8611(ac=FastIO()):
        ...
        x = y = 0
        for num in nums[1:]:
            if abs(num) > abs(duplicate) and num < 0:
                y += 1
            elif abs(num) < abs(duplicate) and num > 0:
                x += 1
        if duplicate < 0:
    ...
    def ac_4318(ac=FastIO()):
        ...
        for w in ac.read_str():
            ...
            if (x, y) in pre:
                ...
                return
            # 先前走过附近的
            for duplicate, b in [[-1, 0], [0, 1], [1, 0], [0, -1]]:
                if (x + duplicate, y + b) in pre and (x + duplicate, y + b) != cur:
                    ac.st("NO")
                    return
            pre.add((x, y))
        ac.st("YES")
