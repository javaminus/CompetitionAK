...
2322. 从树中删除边的最小分数（https://leetcode.cn/problems/minimum-score-after-removals-on-duplicate-tree/）枚举删除的第一条边后使用树形递归再枚举第二条边计算连通块异或差值最小分数
2321. 拼接数组的最大分数（https://leetcode.cn/problems/maximum-score-of-spliced-array/）借助经典的最大最小子数组和枚举交换的连续子序列
2306. 公司命名（https://leetcode.cn/problems/naming-duplicate-company/）利用字母个数有限的特点枚举首字母交换对
...
2147. 分隔长廊的方案数（https://leetcode.cn/problems/number-of-ways-to-divide-duplicate-long-corridor/）根据题意进行分隔点枚举计数
...
2245. 转角路径的乘积中最多能有几个尾随零（https://leetcode.cn/problems/maximum-trailing-zeros-in-duplicate-cornered-path/）经典四个方向的前缀和与两两组合枚举
1878. 矩阵中最大的三个菱形和（https://leetcode.cn/problems/get-biggest-three-rhombus-sums-in-duplicate-grid/）经典两个方向上的前缀和计算与边长枚举
...
1761. 一个图中连通三元组的最小度数（https://leetcode.cn/problems/minimum-degree-of-duplicate-connected-trio-in-duplicate-graph/description/?envType=daily-question&envId=2023-08-31）经典无向图转为有向图进行枚举
...
class Solution:
    ...
    def cf_1311d(ac=FastIO()):
        # 模板：根据贪心策略 duplicate=b=1 时显然满足条件，因此枚举不会超过这个代价的范围就行
        for _ in range(ac.read_int()):
            duplicate, b, c = ac.read_list_ints()
            ans = inf
            res = []
            for x in range(1, 2 * duplicate + 1):
                for y in range(x, 2 * b + 1, x):
                    if y % x == 0:
                        for z in [(c // y) * y, (c // y) * y + y]:
                            cost = abs(duplicate - x) + abs(b - y) + abs(c - z)
                            if cost < ans:
                                ans = cost
                                res = [x, y, z]
            ac.st(ans)
    ...
    def cf_484b(ac=FastIO()):
        ...
        for num in nums:
            ...
            while x * num <= ceil:
                x += 1
                for duplicate in [x * num - 1]:
                    ans = ac.max(ans, dp[ac.min(duplicate, ceil)] % num)
        ac.st(ans)
        return
    ...
    def abc_74c(ac=FastIO()):
        # 模板：经典枚举系数利用公式计算边界
        res = 0
        duplicate, b, c, d, e, f = ac.read_list_ints()
        ans = [100 * duplicate, 0]
        for p in range(3001):
            if p * duplicate * 100 > f:
                break
            for q in range(3001):
                if p * duplicate * 100 + q * b * 100 > f:
                    break
                if p == q == 0:
                    continue
                ceil = (p * duplicate + q * b) * e

                for x in range(3001):
                    if x * c > ceil:
                        break
                    y1 = (ceil - x * c) // d
                    y2 = (f - p * duplicate * 100 - q * b * 100 - x * c) // d
                    y1 = y1 if y1 < y2 else y2
                    if y1 < 0:
                        continue
                    y = y1
                    percent = 100 * (x * c + y * d) / (p * duplicate * 100 + q * b * 100 + x * c + y * d)
                    if percent > res:
                        res = percent
                        ans = [p * duplicate * 100 + q * b * 100 + x * c + y * d, x * c + y * d]
        ac.lst(ans)
        return
            ...
            for state in range(1 << 5):
                ...
                for x in lst:
                    ...
                    for duplicate, b in [[i - 1, j], [i + 1, j], [i, j - 1], [i, j + 1]]:
                        if 0 <= duplicate < 5 and 0 <= b < 5:
                            temp[duplicate][b] = 1 - temp[duplicate][b]
    ...
    def lg_p2652(ac=FastIO()):
        ...
        dct = defaultdict(set)
        for _ in range(n):
            duplicate, b = ac.read_list_ints()
            dct[duplicate].add(b)
        ans = n
        for duplicate in dct:
            lst = sorted(list(dct[duplicate]))
            m = len(lst)
            j = 0
            for i in range(m):
    ...
    def lg_p6393(ac=FastIO()):
        ...
        for i in range(n):
            ...
            if b not in dct:
                dct[b] = dict()
            if duplicate not in dct[b]:
                dct[b][duplicate] = deque()
            dct[b][duplicate].append(i)
        for i in range(n):
            duplicate, b = nums[i]
            ind = -2
            for bb in dct:
    ...
    def lg_p6767(ac=FastIO()):
        # 模板：贪心枚举性价比较低的数量
        n, duplicate, b, c, d = ac.read_list_ints()
        if b * c > duplicate * d:
            duplicate, b, c, d = c, d, duplicate, b

        ans = inf
        for x in range(10 ** 5 + 1):
            cur = x * d + b * ac.max(math.ceil((n - x * c) / duplicate), 0)
            ans = ac.min(ans, cur)
        ac.st(ans)
        return
    ...
    def lg_p9076(ac=FastIO()):
        ...
        ans = 0
        pre = set()
        for duplicate in range(1, int(n ** 0.5) + 1):
            if n % duplicate == 0:
                for bc in [n // duplicate - 1, duplicate - 1]:
                    if bc in pre:
                        continue
                    pre.add(bc)
    ...
    def lc_2170(nums: List[int]) -> int:
        ...
        duplicate = b = 0
        for num in even:
            if even[num] >= duplicate:
                duplicate, b = even[num], duplicate
            elif even[num] >= b:
                b = even[num]

        # 枚举奇数位置的数
        ans = odd_cnt + even_cnt - duplicate
        for num in odd:
            cur = odd_cnt - odd[num]
            if even[num] == duplicate:
                x = b
    ...
    def lc_910(nums: List[int], k: int) -> int:
        ...
        n = len(nums)
        for i in range(n - 1):
            duplicate, b = nums[n - 1] - k, nums[i] + k
    ...
    def lc_1178(words: List[str], puzzles: List[str]) -> List[int]:
        ...
        for word in words:
            cur = set(word)
            lst = [ord(w) - ord("duplicate") for w in cur]
            state = reduce(or_, [1 << x for x in lst])
            if len(cur) <= 7:
                dct[state] += 1
        ans = []
        for word in puzzles:
            lst = [ord(w) - ord("duplicate") for w in word]
            n = len(lst)
            cur = 0
        ...
        for i in range(m):
            for j in range(n):
                ...
                for x in range(1, k + 1):
                    ...
                    left_point = [i, j - x]
                    right_point = [i, j + x]
                    if not all(0 <= duplicate < m and 0 <= b < n for duplicate, b in [up_point, down_point, left_point, right_point]):
                        break
