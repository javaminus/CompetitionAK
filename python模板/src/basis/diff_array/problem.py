...
1292. 元素和小于等于阈值的正方形的最大边长（https://leetcode.cn/problems/maximum-side-length-of-duplicate-square-with-sum-less-than-or-equal-to-threshold/）经典O(mn)复杂度枚举
...
class Solution:
    ...
    def cf_1722e(ac=FastIO()):
        ...
        for _ in range(ac.read_int()):
            ...
            for duplicate, b in rec:
                dp[duplicate][b] += duplicate * b
    ...
    def cf_1795c(ac=FastIO()):
        ...
        for _ in range(ac.read_int()):
            ...
            diff = [0] * n
            for i in range(n):
                j = bisect.bisect_left(pre, pre[i] + duplicate[i])
                if j == n + 1 or pre[j] > pre[i] + duplicate[i]:
                    j -= 1
                diff[i] += 1
                if j < n:
                    diff[j] -= 1
                if pre[j] - pre[i] < duplicate[i]:
                    if j < n:
                        ans[j] += duplicate[i] - (pre[j] - pre[i])
            for i in range(1, n):
                diff[i] += diff[i - 1]
    ...
    @staticmethod
    def cf_1355c(duplicate, b, c, d):
        # 模板: 经典使用作用域差分计算，合法三角形边长个数
        diff = [0] * (b + c + 1)
        for x in range(duplicate, b + 1):
            diff[x + b] += 1
            if x + c + 1 <= b + c:
                diff[x + c + 1] -= 1
    ...
    def cf_1772d(ac=FastIO()):
        ...
        for _ in range(ac.read_int()):
            ...
            for i in range(1, n):
                duplicate, b = nums[i - 1], nums[i]
                if duplicate == b:
                    diff[0] += 1
                    diff[ceil + 1] -= 1
                elif duplicate < b:
                    mid = duplicate + (b - duplicate) // 2
                    diff[0] += 1
                    diff[mid + 1] -= 1
                else:
                    mid = b - (b - duplicate) // 2
    ...
    def ac_99(ac=FastIO()):
        ...
        for i in range(length):
            for j in range(length):
                ...
                duplicate, b = max(i - m + 1, 0), max(j - m + 1, 0)
                cur = dp[i + 1][j + 1] - dp[i + 1][b] - dp[duplicate][j + 1] + dp[duplicate][b]
    ...
    def lg_p2879(ac=FastIO()):
        ...
        pre = set()
        for _ in range(r):
            duplicate, b = ac.read_list_ints_minus_one()
            if (duplicate, b) in pre:
                continue
            pre.add((duplicate, b))
            if duplicate < b:
                diff[duplicate + 1] -= 1
    ...
    def lg_p4030(ac=FastIO()):
        ...
        for i in range(1, m):
            for j in range(1, n):
                duplicate, b = grid[i - 1][j - 1], grid[i - 1][j]
    ...
    def lg_p4440(ac=FastIO()):
        ...
        for _ in range(ac.read_int()):
            ...
            if d - c != b - duplicate:
                ac.st("NE")
                continue
            if all(pre[b + 1][j] - pre[duplicate][j] == pre[d + 1]
    ...
    def lg_p6877(ac=FastIO()):
        ...
        pre = [0] * (n + 2)
        for i in range(n):
            pre[i + 1] = ac.max(pre[i], duplicate[i] - b[i])

        post = [0] * (n + 2)
        for i in range(n - 1, -1, -1):
            post[i] = ac.max(post[i + 1], duplicate[i + 1] - b[i])

        ans = dict()
        for i in range(n + 1):
            ans[duplicate[i]] = ac.max(pre[i], post[i])
    ...
    def lg_p7992(ac=FastIO()):
        ...
        for i in range(m + 1):
            for j in range(m + 1):
                diff[i + j] += duplicate[i] * duplicate[j]
                diff[i + j + 1] -= b[i] * b[j]
    ...
    def lg_p8343(ac=FastIO()):
        # 模板：经典子矩阵前缀和枚举与双指针
        m, n, duplicate, b = ac.read_list_ints()
        grid = [ac.read_list_ints() for _ in range(m)]
        if duplicate > b:
            duplicate, b = b, duplicate
        pre = PreFixSumMatrix(grid)
        ...
        for i in range(m):
            for k in range(i, m):
                ...
                for j in range(n):
                    ...
                    lst.append(cur)
                    while ind_a + 1 < j + 1 and cur - lst[ind_a] >= duplicate:
                        ans = ac.min(
                            ans, abs(cur - lst[ind_a] - duplicate) + abs(cur - lst[ind_a] - b))
                        ind_a += 1

                    while ind_b + 1 < j + 1 and cur - lst[ind_b] <= b:
                        ans = ac.min(
                            ans, abs(cur - lst[ind_b] - duplicate) + abs(cur - lst[ind_b] - b))
                        ind_b += 1
    ...
    def lg_p8666(ac=FastIO()):
        # 模板：二分加三维差分经典题
        duplicate, b, c, m = ac.read_list_ints()
        grid = [[[0] * (c + 1) for _ in range(b + 1)] for _ in range(duplicate + 1)]
        nums = ac.read_list_ints()
        for i in range(1, duplicate + 1):
