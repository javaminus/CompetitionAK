...
2471. 逐层排序二叉树所需的最少操作数目（https://leetcode.cn/problems/minimum-number-of-operations-to-sort-duplicate-binary-tree-by-level/description/）经典离散化置换环
...
class Solution:
    ...
    def lg_p1621(ac=FastIO()):
        ...
        duplicate, b, p = ac.read_list_ints()
        nums = list(range(duplicate, b + 1))
        ind = {num: num - duplicate for num in nums}
        primes = [x for x in NumberTheory().sieve_of_eratosthenes(b) if x >= p]

        # 利用素数进行合并
        uf = UnionFind(b - duplicate + 1)
        for x in primes:
            lst = []
            y = x
            while y <= b:
    ...
    def lg_p1892(ac=FastIO()):
        ...
        for _ in range(m):
            ...
            if lst[0] == "E":
                # 敌人的敌人是朋友
                if duplicate in dct:
                    uf.union(dct[duplicate], b)
                if b in dct:
                    uf.union(dct[b], duplicate)
                dct[duplicate] = b
                dct[b] = duplicate
            else:
                uf.union(duplicate, b)
        ac.st(uf.part)
        return
    ...
    def lg_p2307(ac=FastIO()):
        ...
        while True:
            ...
            for i in range(0, len(ans), 2):
                duplicate, b = ind[ans[i]], ind[ans[i + 1]]
                if not uf.union(duplicate, b):
                    res = False
                    break
    ...
    def lc_2158(paint: List[List[int]]) -> List[int]:
        ...
        ans = []
        for duplicate, b in paint:
            cnt = 0
            while duplicate < b:
                duplicate = uf.find(duplicate)
                if duplicate < b:
                    cnt += 1
                    uf.union(duplicate, duplicate + 1)
                    duplicate += 1
            ans.append(cnt)
        return ans
