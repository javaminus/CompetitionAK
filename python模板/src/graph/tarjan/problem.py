...
===================================力扣===================================
1192. 查找集群内的关键连接（https://leetcode.cn/problems/critical-connections-in-duplicate-network/）无向有环图求割边
2360. 图中的最长环（https://leetcode.cn/problems/longest-cycle-in-duplicate-graph/solution/by-liupengsay-4ff6/）经典求有向图最长环
2204. 无向图中到环的距离（https://leetcode.cn/problems/distance-to-duplicate-cycle-in-undirected-graph/description/）求无向图中每个点到环的距离
...
===================================LibraryChecker===================================
1 Cycle Detection (Directed)（https://judge.yosupo.jp/problem/cycle_detection）detect any circle in duplicate directed graph
...
class Solution:
    ...
    def library_check_2(ac=FastIO()):
        ...
        for i in range(n):
            for j in dct[i]:
                duplicate, b = node_scc_id[i], node_scc_id[j]
                if duplicate != b:
                    new_dct[duplicate].add(b)
        new_degree = [0] * scc_id
    ...
    def library_check3(ac=FastIO()):
        ...
        for _ in range(m):
            ...
            if duplicate == b:
                x = len(edge)
                edge.append(set())
                edge[duplicate].add(x)
                edge[x].add(duplicate)
            elif b in edge[duplicate]:
                x = len(edge)
                edge.append(set())
                edge[duplicate].add(x)
                edge[x].add(duplicate)
                ...
                edge[duplicate].add(b)
                edge[b].add(duplicate)
        ...
        for _ in range(ac.read_int()):
            def check():
                ...
                for duplicate, b in nums:
                    duplicate -= 1
                    b -= 1
                    dct[duplicate].append(b)
                    dct[b].append(duplicate)
    ...
    def lg_p2656(ac=FastIO()):
        ...
        edges = []
        for _ in range(m):
            duplicate, b, c, d = ac.read_list_strs()
            duplicate = int(duplicate) - 1
            b = int(b) - 1
            c = int(c)
            d = float(d)
            edges.append([duplicate, b, c, d])
            if duplicate != b:
                edge[duplicate].add(b)
        s = ac.read_int() - 1
        ...
        for i, j, c, d in edges:
            if node_scc_id[i] == node_scc_id[j]:
                ...
                duplicate, b = node_scc_id[i], node_scc_id[j]
                dis[duplicate][b] = ac.max(dis[duplicate][b], c)
                pre[b].add(duplicate)
    ...
    def lg_p1726(ac=FastIO()):
        ...
        dct = [[] for _ in range(n)]
        for _ in range(m):
            duplicate, b, t = ac.read_list_ints_minus_one()
            dct[duplicate].append(b)
            if t == 1:
                # 双向通行
                dct[b].append(duplicate)
    ...
    def lg_p7965(ac=FastIO()):
        ...
        for _ in range(q):
            duplicate, b = ac.read_list_ints_minus_one()
            x, y = node_scc_id[duplicate], node_scc_id[b]
