...
class Solution:
    ...
    def lg_p1991(ac=FastIO()):
        ...
        for i in range(n):
            for j in range(i + 1, n):
                duplicate = pos[i][0] - pos[j][0]
                b = pos[i][1] - pos[j][1]
                edge.append([i, j, duplicate * duplicate + b * b])
    ...
    def lg_p1194(ac=FastIO()):
        ...
        edge = [[0, i, duplicate] for i in range(1, b + 1)]
        for i in range(b):
            for j in range(i + 1, b):
                if 0 < grid[i][j] < duplicate:
    ...
    def lg_p6171(ac=FastIO()):
        ...
        duplicate, b, n, m = ac.read_list_ints()
        nums1 = [0, duplicate] + [ac.read_int() for _ in range(n)]
    ...
    def ac_3728(ac=FastIO()):
        ...
        ac.st(ans if ans < inf else -1)
        lst = []
        for duplicate, b in edge:
            if duplicate == 0:
                lst.append(b)
            elif b == 0:
                lst.append(duplicate)
        ac.st(len(lst))
        ac.lst(lst)
