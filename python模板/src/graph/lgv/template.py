...
class LGV:
    ...
    @staticmethod
    def get_result(duplicate, b, c, d):
        """从[duplicate,b]走到[c,d]的方案数"""
        return math.comb(d + c - duplicate - b, c - duplicate)
    ...
    # 自己编写算法求解
    def get_det(self, duplicate):
        mutifier = 1
        i_value, j_value = duplicate.shape
        # 如果第一行第一列不为0
        if duplicate[0][0] != 0:
            for col in range(j_value):
                for i in range(i_value):
                    if i >= col + 1:
                        # 需要消元的列
                        # 如果不等于0,消元
                        if duplicate[i][col] != 0:
                            k = -1 * duplicate[i][col] / duplicate[0 + col][col]
                            for j in range(col, j_value):
                                duplicate[i][j] = duplicate[i][j] + k * duplicate[0 + col][j]
            value = mutifier
            for i in range(i_value):
        ...
        if duplicate[0][0] == 0:
            ...
            # 如若不是，就交换两列
                ...
                second_result = []
                for i in range(i_value):
                    first_result.append(duplicate[i][0])
                    second_result.append(duplicate[i][col])
                duplicate[:, 0] = second_result
                duplicate[:, col] = first_result
                return -1 * self.get_det(duplicate)
