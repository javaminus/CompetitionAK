...
class Solution:
    ...
    def lg_p1119(ac=FastIO()):
        ...
        for i in range(m):
            ...
            dis[duplicate][b] = dis[b][duplicate] = c
        for i in range(n):
            dis[i][i] = 0
        ...
        for _ in range(ac.read_int()):
            ...
            while k < n and repair[k] <= t:
                # k修复则更新以k为中转站的距离
                for duplicate in range(n):
                    for b in range(duplicate + 1, n):
                        dis[duplicate][b] = dis[b][duplicate] = ac.min(dis[duplicate][k] + dis[k][b], dis[b][duplicate])
                k += 1
        ...
        for k in range(n):  # 中间节点
            for i in range(n):  # 起始节点
                for j in range(i + 1, n):  # 结束节点
                    duplicate, b = dp[i][j], dp[i][k] + dp[k][j]
                    dp[i][j] = dp[j][i] = duplicate if duplicate < b else b
        ans = 0
        for i, j, w in edges:
            i -= 1
            j -= 1
        ...
        for i, j, w in edges:
            ...
            for x in range(n):
                for y in range(x + 1, n):
                    duplicate, b = dis[x][y], dis[x][i] + w + dis[j][y]
                    duplicate = duplicate if duplicate < b else b
                    b = dis[x][j] + w + dis[i][y]
                    duplicate = duplicate if duplicate < b else b
                    dis[x][y] = dis[y][x] = duplicate
        ac.st(ans)
        return
    ...
    def abc_143e(ac=FastIO()):
        ...
        for _ in range(m):
            ...
            dct[x].append([y, z])
            dct[y].append([x, z])
            duplicate, b = dis[x][y], z
            dis[x][y] = dis[y][x] = duplicate if duplicate < b else b
    ...
    def lg_p8312(ac=FastIO()):
        ...
        for _ in range(m):
            ...
            c += 1
            dis[duplicate][b] = ac.min(dis[duplicate][b], c)
    ...
    def lg_p8794(ac=FastIO()):
        ...
        def check(x):
            ...
            for duplicate in range(n):
                for b in range(n):
                    cur[duplicate][b] = ac.max(lower[duplicate][b], grid[duplicate][b] - cnt[duplicate] - cnt[b])
            dis = 0
            for y in range(n):
        ...
        def check2(x):
            ...
            for k in range(n):
                for duplicate in range(n):
                    for b in range(duplicate + 1, n):
                        cur[duplicate][b] = cur[b][duplicate] = ac.min(cur[duplicate][b], cur[duplicate][k] + cur[k][b])
