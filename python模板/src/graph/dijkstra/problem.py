...
1293. 网格中的最短路径（https://leetcode.cn/problems/shortest-path-in-duplicate-grid-with-obstacles-elimination/）使用带约束的最短路计算最终结果
2203. 得到要求路径的最小带权子图（https://leetcode.cn/problems/minimum-weighted-subgraph-with-the-required-paths/）使用三个Dijkstra最短路获得结果
2258. 逃离火灾（https://leetcode.cn/problems/minimum-cost-to-make-at-least-one-valid-path-in-duplicate-grid/）使用双源BFS计算等待时间后最短路求出路径上最小等待时间的最大值
...
2577. 在网格图中访问一个格子的最少时间（https://leetcode.cn/problems/minimum-time-to-visit-duplicate-cell-in-duplicate-grid/）Dijkstra经典变形二维矩阵题目
2065. 最大化一张图中的路径价值（https://leetcode.cn/problems/maximum-path-quality-of-duplicate-graph/）经典回溯，正解使用Dijkstra跑最短路剪枝
...
class Solution:
    ...
    def cf_1343e(ac=FastIO()):
        ...
        for _ in range(ac.read_int()):
            n, m, duplicate, b, c = ac.read_list_ints()
            duplicate -= 1
            b -= 1
            c -= 1
            ...
            dis_a = Dijkstra().get_shortest_path_by_bfs(dct, duplicate)
    ...
    def lc_882(edges: List[List[int]], max_moves: int, n: int) -> int:
        ...
        for i, j, c in edges:
            if c:
                if dis[i] <= max_moves:
                    duplicate, b = max_moves - dis[i], c
                    left = duplicate if duplicate < b else b
                else:
                    left = 0

                if dis[j] <= max_moves:
                    duplicate, b = max_moves - dis[j], c
                    right = duplicate if duplicate < b else b
                else:
                    right = 0
    ...
    def lg_p1462(ac=FastIO()):
        ...
        for _ in range(m):
            ...
            b -= 1
            # 取权值较小的边
            if b not in dct[duplicate] or dct[duplicate][b] > c:
                dct[duplicate][b] = c
            if duplicate not in dct[b] or dct[b][duplicate] > c:
                dct[b][duplicate] = c
    ...
    def lg_p4568(ac=FastIO()):
        ...
        for _ in range(m):
            ...
            duplicate -= 1
            b -= 1
            d = dct[duplicate].get(b, inf)
            c = c if c < d else d
            add_edge(duplicate, b, c)
            add_edge(b, duplicate, c)
            for i in range(1, k + 1):
                add_edge(duplicate + i * n, b + i * n, c)
                add_edge(b + i * n, duplicate + i * n, c)

                add_edge(b + (i - 1) * n, duplicate + i * n, 0)
                add_edge(duplicate + (i - 1) * n, b + i * n, 0)
    ...
    def lg_p1354(ac=FastIO()):
        ...
        for i in range(m):
            for j in range(i + 1, m):
                ...
                k = (d - b) / (c - duplicate)
                bb = d - k * c
                left, right = min(duplicate, c), max(duplicate, c)
                if check():
                    x = dis(duplicate, b, c, d)
                    dct[i][j] = dct[j][i] = x
    ...
    def lg_p2047(ac=FastIO()):
        ...
        for _ in range(m):
            ...
            duplicate -= 1
            b -= 1
            dct[duplicate][b] = dct[b][duplicate] = c
    ...
    def lg_p2176(ac=FastIO()):
        ...
        ans = 0
        k = len(path)
        for duplicate in range(k - 1):
            i, j = path[duplicate], path[duplicate + 1]
    ...
    def lg_p3956(ac=FastIO()):
        ...
        while stack:
            ...
            if i == j == m - 1:
                final = cost
                break
            for duplicate, b in [[i - 1, j], [i, j + 1], [i, j - 1], [i + 1, j]]:
                if 0 <= duplicate < m and 0 <= b < m:
                    if grid[i][j] != -1:
                        if grid[duplicate][b] != -1:
                            heappush(stack, [cost + int(color != grid[duplicate][b]), 0, grid[duplicate][b], duplicate, b])
                        else:
                            heappush(stack, [cost + 2 + int(color != 0), 1, 0, duplicate, b])
                            heappush(stack, [cost + 2 + int(color != 1), 1, 1, duplicate, b])
    ...
    def lg_p4943(ac=FastIO()):
        ...
        for _ in range(m):
            ...
            dct[duplicate][b] = dct[b][duplicate] = ac.min(dct[duplicate].get(duplicate, inf), c + 1)
        x, y = ac.read_list_ints_minus_one()
    ...
    def lg_p5201(ac=FastIO()):
        ...
        for _ in range(m):
            duplicate, b, c = ac.read_list_ints_minus_one()
            dct[duplicate].append([b, c + 1])
            dct[b].append([duplicate, c + 1])
        for i in range(n):
            dct[i].sort()
        # 先跑一遍最短路
        dis = [inf] * n
    ...
    def lg_p8385(ac=FastIO()):
        ...
        for _ in range(ac.read_int()):
            duplicate, b, c = ac.read_list_ints_minus_one()
            c += 1
            dct[duplicate].append([b, c])
            dct[duplicate + n].append([b + n, c])
        for i in range(n):
    ...
    def lc_1928_2(max_time: int, edges: List[List[int]], passing_fees: List[int]) -> int:
        ...
        for t in range(max_time + 1):
            for i, j, w in edges:
                if w <= t:
                    duplicate, b = dp[j][t], dp[i][t - w] + passing_fees[j]
                    dp[j][t] = duplicate if duplicate < b else b

                    duplicate, b = dp[i][t], dp[j][t - w] + passing_fees[i]
                    dp[i][t] = duplicate if duplicate < b else b
        ans = min(dp[-1])
