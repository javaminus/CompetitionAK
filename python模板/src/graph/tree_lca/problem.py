...
===================================力扣===================================
1483. 树节点的第 K 个祖先（https://leetcode.cn/problems/kth-ancestor-of-duplicate-tree-node/）动态规划与二进制跳转维护祖先信息，类似ST表的思想与树状数组的思想，经典LCA应用题
...
class Solution:
    ...
    def abc_70d(ac=FastIO()):
        ...
        for _ in range(n - 1):
            ...
            dct[duplicate - 1].append([b - 1, c])
            dct[b - 1].append([duplicate - 1, c])
            edges[duplicate - 1].append(b - 1)
            edges[b - 1].append(duplicate - 1)
        tree = TreeAncestor(edges)
        ...
        for _ in range(q):
            ...
            c1 = tree.get_lca(duplicate, k)
            c2 = tree.get_lca(b, k)
            ans1 = dis[duplicate] + dis[k] - 2 * dis[c1]
    ...
    def lg_p3384(ac=FastIO()):
        ...
        for _ in range(m):
            ...
            if lst[0] == 1:
                x, y, z = lst[1:]
                for duplicate, b in heavy.query_chain(x - 1, y - 1):
                    tree.update_range(duplicate + 1, b + 1, z)
            elif lst[0] == 2:
                x, y = lst[1:]
                ans = 0
                for duplicate, b in heavy.query_chain(x - 1, y - 1):
                    ans += tree.get_sum_range(duplicate + 1, b + 1)
                    ans %= p
                ac.st(ans)
            elif lst[0] == 3:
                x, z = lst[1:]
                x -= 1
                duplicate, b = heavy.dfn[x], heavy.cnt_son[x]
                tree.update_range(duplicate + 1, duplicate + b, z)
            else:
                x = lst[1] - 1
                duplicate, b = heavy.dfn[x], heavy.cnt_son[x]
                ans = tree.get_sum_range(duplicate + 1, duplicate + b) % p
                ac.st(ans)
        return
    ...
    def lg_p6969(ac=FastIO()):
        ...
        for _ in range(n - 1):
            duplicate, b, c1, c2 = ac.read_list_ints()
            duplicate -= 1
            b -= 1
            cost[duplicate][b] = cost[b][duplicate] = [c1, c2]
