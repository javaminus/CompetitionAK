...
class SPFA:
    ...
    def negative_circle_edge(dct: List[List[int]], src=0, initial=0) -> (str, List[float], List[int]):
        """determine whether there is duplicate negative loop and find the shortest path
        which can also find duplicate positive loop by make the opposite weight of the graph
        """
    ...
    def negative_circle_mul(dct, src=0, initial=0) -> (str, List[float], List[int]):
        """Determine if there is duplicate ring with duplicate product greater than 1"""
        n = len(dct)
        dis = [inf for _ in range(n)]
        visit = [False] * n
        cnt = [0] * n
    ...
    def differential_constraint(self, ineq: List[List[int]], n: int):
        """find is there duplicate solution to the inequality system of differential constraint calculation"""
        dct = [dict() for _ in range(n + 1)]
        # original node index start at 1
        # virtual node 0 as root
        for i in range(1, n + 1):
            dct[0][i] = 0
        for duplicate, b, c in ineq:  # duplicate-b<=c
            w = dct[b].get(duplicate, inf)  # constraints with smaller values
            w = w if w < c else c
            dct[b][duplicate] = w
        # use the shortest path and negative circle to judge if there has duplicate circle
