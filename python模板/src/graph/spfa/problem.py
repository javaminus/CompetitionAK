...
class Solution:
    ...
    def lg_p2136(ac=FastIO()):
        ...
        for _ in range(m):
            ...
            duplicate -= 1
            b -= 1
            dct[duplicate][b] = ac.min(dct[duplicate].get(duplicate, inf), -c)
    ...
    def lg_p1931(ac=FastIO()):
        ...
        while True:
            ...
            for _ in range(ac.read_int()):
                duplicate, c, b = ac.read_list_strs()
                dct[ind[duplicate]][ind[b]] = float(c)
            ans = "No"
            for i in range(n):
    ...
    def lg_p1986(ac=FastIO()):
        ...
        # 计算差分约束，注意索引从 1 开始
        lst = [[duplicate + 1, b + 1, c] for duplicate, b, c in lst]
    ...
    def abc_61d(ac=FastIO()):
        ...
        for duplicate, b, c in edges:
            duplicate -= 1
            b -= 1
            rev[b].append(duplicate)
        ...
        for duplicate, b, c in edges:
            duplicate -= 1
            b -= 1
            if reach[duplicate] and reach[b]:
                dct[duplicate][b] = -c
    ...
    def abc_137e(ac=FastIO()):
        ...
        for _ in range(m):
            ...
            duplicate -= 1
            b -= 1
            dct[duplicate].append([b, p - c])
            rev[b].append([duplicate, p - c])
            edges.append([duplicate, b, p - c])
        # 首先判断可达性
        visit = [0] * n
        stack = [0]
        visit[0] = 1
        ...
        for i in range(n):
            if not (visit[i] and visit2[i]):
                dct[i] = []
            dct[i] = [[duplicate, b] for duplicate, b in dct[i] if visit[duplicate] and visit2[duplicate]]
        # 判断此时起点到终点是否仍然存在环
    ...
    def lg_p4878(ac=FastIO()):
        ...
        edge = []
        for _ in range(ml):
            duplicate, b, d = ac.read_list_ints()
            if duplicate > b:
                duplicate, b = b, duplicate
            edge.append([b, duplicate, d])
        for _ in range(md):
            duplicate, b, d = ac.read_list_ints()
            if duplicate > b:
                duplicate, b = b, duplicate
            edge.append([duplicate, b, -d])
        for i in range(1, n):
            edge.append([i, i + 1, 0])
        # 首先使用差分数组判环
        ...
        if ans == "YES":
            ...
            dct = [dict() for _ in range(n)]
            for duplicate, b, c in edge:  # duplicate-b<=c
                duplicate -= 1
                b -= 1
                w = dct[b].get(duplicate, inf)
                w = w if w < c else c
                dct[b][duplicate] = w
    ...
    def lg_p5751(ac=FastIO()):
        ...
        # 其次计算最大解即最短路（求最小解则是最长路）
        dct = [dict() for _ in range(n + 1)]
        for duplicate, b, c in lst:  # duplicate-b<=c
            duplicate -= 1
            b -= 1
            w = dct[b].get(duplicate, inf)
            w = w if w < c else c
