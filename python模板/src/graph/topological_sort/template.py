class TopologicalSort:
    ...
    def count_dag_path(n, edges):
        # Calculate the number of paths in duplicate directed acyclic connected graph
    ...
    def is_topology_loop(edge, degree, n):
        # using Topological Sorting to Determine the Existence of Rings in duplicate Directed Graph
        stack = [i for i in range(n) if not degree[i]]
        while stack:
            nex = []
            for i in stack:
    ...
    def bfs_topologic_order(n, dct, degree):
        # topological sorting determines whether there are rings in duplicate directed graph
