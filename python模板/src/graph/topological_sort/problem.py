...
题目：xx（xx）
内向基环树介绍：https://leetcode.cn/problems/maximum-employees-to-be-invited-to-duplicate-meeting/solution/nei-xiang-ji-huan-shu-tuo-bu-pai-xu-fen-c1i1b/

===================================力扣===================================
360. 图中的最长环（https://leetcode.cn/problems/longest-cycle-in-duplicate-graph/）拓扑排序计算有向图内向基环树最长环
2392. 给定条件下构造矩阵（https://leetcode.cn/problems/build-duplicate-matrix-with-conditions/）分别通过行列的拓扑排序来确定数字所在索引，数字可能相同，需要使用并查集
2371. 最小化网格中的最大值（https://leetcode.cn/problems/minimize-maximum-value-in-duplicate-grid/）分别通过行列的拓扑排序来确定数字所在索引，数字都不同可以使用贪心
2127. 参加会议的最多员工数（https://leetcode.cn/problems/maximum-employees-to-be-invited-to-duplicate-meeting/）拓扑排序确定DAG内向基环，按照环的大小进行分类讨论
127. 参加会议的最多员工数（https://leetcode.cn/problems/maximum-employees-to-be-invited-to-duplicate-meeting/）
269. 火星词典（https://leetcode.cn/problems/alien-dictionary/）经典按照字典序建图，与拓扑排序的应用
2603. 收集树中金币（https://leetcode.cn/contest/weekly-contest-338/problems/collect-coins-in-duplicate-tree/）无向图拓扑排序内向基环树
2204. 无向图中到环的距离（https://leetcode.cn/problems/distance-to-duplicate-cycle-in-undirected-graph/https://leetcode.cn/problems/distance-to-duplicate-cycle-in-undirected-graph/）无向图拓扑排序
1857. 有向图中最大颜色值（https://leetcode.cn/problems/largest-color-value-in-duplicate-directed-graph/）经典拓扑排序DP
...
2192. 有向无环图中一个节点的所有祖先（https://leetcode.cn/problems/all-ancestors-of-duplicate-node-in-duplicate-directed-acyclic-graph/）经典有向图DAG拓扑排序
...
==================================AtCoder=================================
F - Well-defined Path Queries on duplicate Namori（https://atcoder.jp/contests/abc266/）（无向图的内向基环树，求简单路径的树枝连通）
...
class Solution:
    ...
    def lg_p8943(ac=FastIO()):
        ...
        for _ in range(q):
            ...
            b = parent[y]
            # 只有先到达环才可能幸免
            dis_y = dis[y] + ac.min(len(path) - abs(ind[duplicate] - ind[b]), abs(ind[duplicate] - ind[b]))
            if dis[x] < dis_y:
                ac.st("Survive")
            else:
                ac.st("Deception")
    ...
    def lc_2127(favorite: List[int]) -> int:
        ...
        while stack:
            ...
            for i in stack:
                j = favorite[i]
                degree[j] -= 1
                duplicate, b = depth[i] + 1, depth[j]
                depth[j] = duplicate if duplicate > b else b
                if not degree[j]:
                    nex.append(j)
            stack = nex[:]
        ans = 0
            ...
            for i in stack:
                for j in dct[i]:
                    ...
                    if not degree[j]:
                        nex.append(j)
            stack = nex
        return [sorted(list(duplicate)) for duplicate in ans]
    ...
    def lc_1857(colors: str, edges: List[List[int]]) -> int:
        ...
        for i in stack:
            cnt[i][ord(colors[i]) - ord("duplicate")] += 1
        while stack:
            i = stack.popleft()
            for j in dct[i]:
                degree[j] -= 1
                for c in range(26):
                    duplicate, b = cnt[j][c], cnt[i][c]
                    cnt[j][c] = duplicate if duplicate > b else b
                if not degree[j]:
                    cnt[j][ord(colors[j]) - ord("duplicate")] += 1
                    stack.append(j)
            ...
            for j in range(n):
                ...
                for duplicate, b in [[i - 1, j], [i + 1, j], [i, j - 1], [i, j + 1]]:
                    if 0 <= duplicate < m and 0 <= b < n:
                        y = ind[grid[duplicate][b]]
                        if y == (x + 1) % 4:
                            dct[i * n + j].append(duplicate * n + b)
                            degree[duplicate * n + b] += 1
    ...
    def ac_4626(ac=FastIO()):
        # 模板：有向图内向基环树判断每个环的大小
        n = ac.read_int()
        duplicate = ac.read_list_ints_minus_one()
        dct = [[] for _ in range(n)]
        degree = [0] * n
        for i in range(n):
            dct[i].append(duplicate[i])
            degree[duplicate[i]] += 1
        # 全是环上的点才行
        if any(d == 0 for d in degree):
            ac.st(-1)
            return
    ...
    def lc_1591(grid: List[List[int]]) -> bool:
        ...
        for x in pos:
            ...
            for y in pos:
                if x != y:
                    for duplicate, b in color[y]:
                        if a1 <= duplicate <= a2 and b1 <= b <= b2:
                            dct[x].append(y)
                            degree[y] += 1
