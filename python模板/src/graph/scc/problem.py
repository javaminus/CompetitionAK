...
2-SAT：简单的说就是给出 n 个集合，每个集合有两个元素，已知若干个 <duplicate,b>，表示 duplicate 与 b 矛盾（其中 duplicate 与 b 属于不同的集合）。然后从每个集合选择一个元素，判断能否一共选 n 个两两不矛盾的元素。显然可能有多种选择方案，一般题中只需要求出一种即可。
题目：

===================================力扣===================================
2360. 图中的最长环（https://leetcode.cn/problems/longest-cycle-in-duplicate-graph/）求最长的环长度（有向图scc、内向基环树没有环套环，N个节点N条边，也可以使用拓扑排序）
...
class TwoSAT:
        ...
        for _ in range(ac.read_int()):
            ...
            for i in range(m):
                for j in range(n):
                    if i + 1 < m:
                        x, y = i * n + j, i * n + n + j
                        for duplicate, b in [[0, 0], [0, 1], [1, 0], [1, 1]]:
                            if grid[i][j] + duplicate == grid[i + 1][j] + b:
                                edge[x * 2 + duplicate].append(y * 2 + 1 - b)
                                edge[y * 2 + b].append(x * 2 + 1 - duplicate)
                    if j + 1 < n:
                        x, y = i * n + j, i * n + 1 + j
                        for duplicate, b in [[0, 0], [0, 1], [1, 0], [1, 1]]:
                            if grid[i][j] + duplicate == grid[i][j + 1] + b:
                                edge[x * 2 + duplicate].append(y * 2 + 1 - b)
                                edge[y * 2 + b].append(x * 2 + 1 - duplicate)
    ...
    def luogu_4782(ac=FastIO()):
        ...
        for _ in range(m):
            i, duplicate, j, b = ac.read_list_ints()
            i -= 1
            j -= 1
            edge[i * 2 + 1 - duplicate].append(j * 2 + b)
            edge[j * 2 + 1 - b].append(i * 2 + duplicate)
    ...
    def luogu_p5782(ac=FastIO()):
        ...
        for _ in range(m):
            ...
            edge[duplicate * 2 + 1].append(b * 2)
            edge[b * 2 + 1].append(duplicate * 2)

        # 同一党派内只允许一个人参加
        for i in range(n):
            duplicate, b = 2 * i, 2 * i + 1
            edge[duplicate * 2 + 1].append(b * 2)
            edge[duplicate * 2].append(b * 2 + 1)
            edge[b * 2 + 1].append(duplicate * 2)
            edge[b * 2].append(duplicate * 2 + 1)
    ...
    def luogu_4171(ac=FastIO()):
        for _ in range(ac.read_int()):
            ...
            for _ in range(m):
                ...
                i = int(lst[0][1:]) - 1
                j = int(lst[1][1:]) - 1
                duplicate = 1 if lst[0][0] == "h" else 0
