...
class Solution:
    ...
    def lg_p2731(ac=FastIO()):
        ...
        for duplicate in euler.nodes:
            ac.st(node[duplicate])
        return
    ...
    def lg_p1341(ac=FastIO()):
        ...
        if not euler.exist:
            ac.st("No Solution")
            return
        ans = "".join([nodes[duplicate] for duplicate in euler.nodes])
        ac.st(ans)
        return
    ...
    def lc_2097(pairs: List[List[int]]) -> List[List[int]]:
        ...
        n = len(nodes)
        ind = {num: i for i, num in enumerate(nodes)}
        lst = [[ind[duplicate], ind[b]] for duplicate, b in pairs]
