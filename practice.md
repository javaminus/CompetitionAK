...
| Problems                                                     | Hints                                |
| ------------------------------------------------------------ | ------------------------------------ |
| 给你一个n个水池，有`n - 1`个挡板，每次可以抽出其中$[l, r]$个水池中间的所有挡板，问第$i$个水池的水量。 | 定义并查集的父节点是能到达的最右水池 |
| 给你n个点，操作一：将点$x$与点$y$连接；操作二：询问点$x$的联通块中第$k$大的元素。 | 并查集+小根堆                        |
|                                                              |                                      |
...
| Problems                                                     | Hints                                                        |
| ------------------------------------------------------------ | ------------------------------------------------------------ |
| 树上异或                                                     | 任意两个节点 的异或值 = 节点u到根节点的异或值 ^ 节点v到根节点的异或值 |
| 树上第k个祖先                                                | 利用倍增，定义$dp[i][j] 表示第i个节点的第2^j个祖先节点是谁$，$p = dp[i][j]; dp[i][j+1] = dp[p][j]$ |
| 树上LCA（最近公共祖先节点）                                  | 先求出每个点的深度$depth[x] < depth[y]$，然后使用**树上第k个祖先**将两个节点放在一层，如果有$x==y$那么说明祖先节点就是$x$；如果两个点不相等，就使用二分的思想，往上面走，如果两个的公共祖先相同，就往下走；如果不同，就往上走！ |
| 给你一些核心节点，求从根节点出发，能到达所有核心节点最少保留的节点数。 | 求$dfs(x)$表示$x$节点及其子节点包含的核心节点数。最后累加所有不为0的节点数。 |
| 树上直径                                                     | 从任意节点出发的最远点为$x$，然后从$x$出发到最远点$y$，$x-y$就是树上直径 |
| 任意点到其他所有点的距离和                                   | 换根dp，$ans[y] = ans[x] + g.length - 2*size[y]$             |
| 树上m条路径加1，求值最大的节点。                             | 树上差分，如果此时$x$和$y$这条路径被+1，那么$diff[x]++，diff[y]++，diff[lca(x, y)]--$，同时如果$dp[lca(x, y)][0]>0,那么diff[dp[lca(x, y)][0]]--$，防止向祖先节点扩散。最后就是无脑$dfs$，$diff[x]+=diff[y]; ans = Math.max(ans, diff[x])$。 |
| 一棵树分成三个联通分量，求`最大异或值 - 最小异或值`的**最小值** | 枚举不是根的两个点，删除这两个点及其父节点形成的边；然后使用时间戳判断树的父子关系。时间戳的写法$in[x] = ++clock; out[x] = clock;$所以在比较$out[x]$的时候可以加等号。 |
| 给你一棵树，找到权值最大的一条路径，并且路径所有节点**颜色不同**，返回**最大路径长度**，以及最大路径的**最少节点数目**。 | 树上滑窗：其实就是$dfs()$里面需要维护什么信息？到达节点$x$那么使用一个$hashmap$存储$<color[x], x到root节点的节点个数>$，然后维护一个栈$dist$，存储$x$的每个祖先节点到$root$节点的距离。然后有最长距离就是$x$节点到$root$节点的距离`减去`最近的重复节点到$root$的距离。 |
| 找到满足1、所有子节点的生命力**小于**当前节点生命力。2、 子结点的生命力无法把结点 i 的生命力 ai **整除**。 | 利用`时间戳`建图，然后将节点按照生命力升序排序，依次升序遍历节点（保证加入的节点都比当前节点小），然后使用`树状数组`维护节点，$L = in[id], R = out[id]$表示区间$[L+1, R]$，然后求出当前节点生命力的除数，从区间$[L+1, R]$中去除存在且能整除的节点。 |
| 在树上删除 k 条边，使删除后的的图中，所有连通块含有节点数的最小值最大。求这个最大值。 | 注意是删除边，不是删除节点。二分判图。                       |
| **二叉树**上一号节点被感染，每回合都会传染给周围的一圈，在每回合前可以删除一个没有被感染的点。求最多剩多少个点不被感染。 | $dp[i]$表示节点$i$被感染，那么以i为根节点最多可以有多少不被病毒感染的点，在$dfs$里面需要遍历两次当前节点的所有节点。【建议去看代码，这里理解通透就是质的提升！！！】树形dp |
| 有的节点被损坏，需要修复。选择最少的到$root$节点的路径。并返回所有修复的最外层节点。 | 维护一个$boolean返回值的dfs()$                               |
