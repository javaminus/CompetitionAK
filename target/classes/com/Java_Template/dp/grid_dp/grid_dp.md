...
[https://leetcode.cn/problems/maximum-strictly-increasing-cells-in-a-matrix/description/?envType=daily-question&envId=2024-06-19](https://leetcode.cn/problems/maximum-strictly-increasing-cells-in-a-matrix/description/?envType=daily-question&envId=2024-06-19)

```java
import java.util.Arrays;

class Solution { // 超时代码，O(M*M*N*N)
    int m,n;
    int[][] mat;
    int[][] memo;
    ...
    private int dfs(int x, int y) {
        ...
        for (int i = 0; i < m; i++) {
            if (mat[i][y] > mat[x][y]) {
                res = Math.max(res, dfs(i, y) + 1);
            }
        }
        for (int i = 0; i < n; i++) {
            if (mat[x][i] > mat[x][y]) {
                res = Math.max(res, dfs(x, i) + 1);
            }
        }
        return memo[x][y] = res;
    }
...
class Solution {
    public int maxIncreasingCells(int[][] mat) {
        ...
        for (List<int[]> pos : map.values()) {
            // 先把所有 f 值都算出来，再更新 rowMax 和 colMax,f:定义 f[i][j] 表示到达 mat[i][j] 时，访问过的单元格的最大数量（包含 mat[i][j]）。那么答案就是所有 f[i][j] 的最大值。
