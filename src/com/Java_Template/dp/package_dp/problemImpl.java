...
public class problemImpl implements problem {
    ...
    public int maxValueOfCoins(List<List<Integer>> piles, int k) { // 细节边界难调
        int[] dp = new int[k + 1];
        int sumN = 0; // 物品数量和
        for (List<Integer> pile : piles) {
            int n = pile.size();
            for (int i = 1; i < n; i++) {
                pile.set(i, pile.get(i - 1) + pile.get(i)); // 前缀和
            }
            sumN += n;
            sumN = Math.min(sumN, k); // 优化：j 从前 i 个栈的大小之和开始枚举（不超过 k）
            for (int j = sumN; j > 0; j--) { // 选择物品数量，背包容量
                for (int w = 0; w < Math.min(n, j); w++) {
                    dp[j] = Math.max(dp[j], dp[j - w - 1] + pile.get(w));
                }
            }
        }
        return dp[k];
